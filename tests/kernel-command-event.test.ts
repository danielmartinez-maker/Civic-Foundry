...
test("command sequences are monotonic and FIFO across command types", () => {
  ...
  assert.deepEqual(
    dispatched.map((item) => item.sequence),
    [1, 2, 3],
  );
...
test("queued command payload is deeply isolated from later external mutation", () => {
  ...
  assert.deepEqual(queued.command.payload, {
    nested: { value: 1 },
    rows: [{ count: 2 }],
  });
...
test("future-tick commands remain pending until eligible", () => {
  ...
  assert.deepEqual(
    bus.pending().map((item) => item.sequence),
    [1, 2],
  );
...
test("command enqueue and dispatch reject invalid ticks and empty types", () => {
  const bus = new CommandBus();
  assert.throws(
    () => bus.enqueue({ type: "", payload: {} }, 0),
    /command type must not be empty/,
  );
  assert.throws(
    () => bus.enqueue({ type: "x", payload: {} }, -1),
    /command tick must be a non-negative integer/,
  );
  assert.throws(
    () => bus.enqueue({ type: "x", payload: {} }, 1.5),
    /command tick must be a non-negative integer/,
  );
  assert.throws(
    () => bus.dispatchReady(-1, context(bus)),
...
test("domain events preserve monotonic sequence and stable append order", () => {
  ...
  const first = journal.append(3, {
    type: "FirmOpened",
    source: "economy",
    payload: { id: "a" },
  });
  const second = journal.append(3, {
    type: "FirmClosed",
    source: "economy",
    payload: { id: "b" },
  });
  ...
  assert.deepEqual(
    journal.list().map((event) => event.type),
    ["FirmOpened", "FirmClosed"],
  );
...
test("event since filtering and diagnostic clear do not rewind sequence", () => {
  ...
  assert.equal(
    journal.append(4, { type: "d", source: "test", payload: {} }).sequence,
    4,
  );
});
...
test("domain event journal validates event identity and tick", () => {
  const journal = new DomainEventJournal();
  assert.throws(
    () => journal.append(-1, { type: "x", source: "test", payload: {} }),
    /event tick must be a non-negative integer/,
  );
  assert.throws(
    () => journal.append(1.5, { type: "x", source: "test", payload: {} }),
    /event tick must be a non-negative integer/,
  );
  assert.throws(
    () => journal.append(1, { type: "", source: "test", payload: {} }),
    /event type must not be empty/,
  );
  assert.throws(
    () => journal.append(1, { type: "x", source: "", payload: {} }),

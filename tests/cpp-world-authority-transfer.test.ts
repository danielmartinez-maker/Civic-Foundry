...
import type {
  DesignStormEvent,
  FloodResult,
} from "../src/world/hydrology/HydrologyTypes.ts";
...
class FakeNativeWorldBridge implements NativeWorldBridge {
  ...
  createLegacyWorld(
    ...
    throw new Error(
      `legacy path not expected in this fixture: ${request.seed}:${request.mode}:${request.terrain.width}`,
    );
  }
  ...
  runDesignStorm(
    ...
    return Object.freeze({
      result,
      snapshot: this.world.snapshotAuthoritative(),
    });
  }
}
...
test("native WorldFoundation authority is a snapshot facade and never reconstructs the TypeScript WorldFoundation", () => {
  ...
  assert.deepEqual(
    ...
    reference.legacyTerrain().snapshot(),
  );
  assert.equal(
    world.preparationMultiplierAt(2, 2),
    reference.preparationMultiplierAt(2, 2),
  );
  ...
  assert.doesNotMatch(
    source,
    /WorldFoundation\.(?:generate|restore|fromLegacyTerrain)/,
  );
});
...
test("Task 19 feature gate makes SimulationCore read the native WorldFoundation snapshot", () => {
  ...
  assert.deepEqual(
    ...
    referenceWorld().snapshotAuthoritative(),
  );
  assert.deepEqual(
    core.terrain.snapshot(),
    core.world.legacyTerrain().snapshot(),
  );
});
...
test("native world authority global gate is explicit and defaults off", () => {
  ...
  assert.equal(
    nativeWorldAuthorityEnabledFromGlobal({
      __CIVIC_NATIVE_WORLD_AUTHORITY__: true,
    }),
    true,
  );
  assert.equal(
    nativeWorldAuthorityEnabledFromGlobal({
      __CIVIC_NATIVE_WORLD_AUTHORITY__: "1",
    }),
    true,
  );
  assert.equal(
    nativeWorldAuthorityEnabledFromGlobal({
      __CIVIC_NATIVE_WORLD_AUTHORITY__: "false",
    }),
    false,
  );
});

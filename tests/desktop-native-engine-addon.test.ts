...
function fakeDesktopApi(): DesktopNativeEngineApi & { calls: string[] } {
  ...
  return {
    ...
    available: () => true,
    createEngine: (config = {}) => {
      calls.push(`create:${JSON.stringify(config)}`);
      return true;
    },
    destroyEngine: () => {
      calls.push("destroy");
    ...
    loadV9: (json) => {
      calls.push(`load:${json}`);
      return true;
    },
    saveV9: () => '{"saveVersion":9}',
    getSnapshot: () =>
      '{"hashVersion":1,"pendingCommands":[],"randomStreams":{},"seed":7,"speed":1,"tick":0}',
    getEvents: () => "[]",
...
test("desktop addon adapts the hardened preload API to NativeEngineBridge", () => {
  ...
  const bridge = new NativeEngineBridge(new DesktopNativeEngineAddon(api), {
    seed: 7,
  });
  bridge.submit([
    {
      sequence: 1,
      tick: 0,
      type: "transport.legacy_roads.replace",
      payload: { revision: 0, cells: [] },
    },
  ]);
  bridge.step(0);
  ...
  bridge.dispose();
  assert.deepEqual(
    api.calls.map((call) => call.split(":", 1)[0]),
    ["create", "submit", "step", "destroy"],
  );
});
...
test("desktop native discovery distinguishes browser fallback from unavailable desktop addon", () => {
  ...
  const scope = {
    __CIVIC_NATIVE_DESKTOP__: { ...unavailable, available: () => false },
  };
  ...
  assert.ok(
    desktopNativeEngineAddonFromGlobal({ __CIVIC_NATIVE_DESKTOP__: available }),
  );
});

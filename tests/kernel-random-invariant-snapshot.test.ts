...
test('named random streams are deterministic and isolated', () => {
  ...
  assert.equal(a.stream('demographics').next(), b.stream('demographics').next());
...
test('different names derive independent deterministic sequences', () => {
  ...
  const traffic = Array.from({ length: 4 }, () => registry.stream('traffic').next());
  const housing = Array.from({ length: 4 }, () => registry.stream('housing').next());
  assert.notDeepEqual(traffic, housing);
  const repeat = new RandomStreamRegistry(99);
  assert.deepEqual(traffic, Array.from({ length: 4 }, () => repeat.stream('traffic').next()));
  assert.deepEqual(housing, Array.from({ length: 4 }, () => repeat.stream('housing').next()));
});
...
test('random stream listing and snapshot keys use ordinal deterministic order', () => {
  ...
  assert.throws(() => registry.stream('   '), /random stream name must not be empty/);
});

test('invariant runner honors cadence and deterministic registration order', () => {
  const runner = new InvariantRunner();
  const seen: string[] = [];
  runner.register({ id: 'zeta', cadence: { every: 2, offset: 0 }, check: ({ tick }) => seen.push(`z:${tick}`) });
  runner.register({ id: 'alpha', cadence: { every: 1 }, check: ({ tick }) => seen.push(`a:${tick}`) });
  runner.runDue(1, context(1));
  runner.runDue(2, context(2));
  assert.deepEqual(seen, ['a:1', 'a:2', 'z:2']);
  assert.deepEqual(runner.list().map((item) => item.id), ['alpha', 'zeta']);
});

test('invariant runner rejects duplicates and invalid cadence', () => {
  const runner = new InvariantRunner();
  runner.register({ id: 'x', cadence: { every: 1 }, check: () => {} });
  assert.throws(() => runner.register({ id: 'x', cadence: { every: 1 }, check: () => {} }), /duplicate invariant: x/);
  assert.throws(() => runner.register({ id: '', cadence: { every: 1 }, check: () => {} }), /invariant id must not be empty/);
  assert.throws(() => runner.register({ id: 'bad', cadence: { every: 0 }, check: () => {} }), /invalid cadence/);
});

test('invariant failure includes invariant id tick and original detail', () => {
  const runner = new InvariantRunner();
  runner.register({
    id: 'population-conservation', cadence: { every: 1 },
    check: () => { throw new Error('population mismatch'); },
  });
  assert.throws(
...
test('snapshot registry rejects duplicate ids and captures in ordinal id order', () => {
  ...
  assert.throws(() => snapshots.register('alpha', () => ({})), /duplicate snapshot provider: alpha/);
  assert.throws(() => snapshots.register('', () => ({})), /snapshot provider id must not be empty/);
  assert.throws(() => snapshots.capture('missing'), /unknown snapshot provider: missing/);
});

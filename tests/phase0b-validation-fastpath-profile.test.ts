...
test('profile remaining Phase 0B sync cost after validation fast path', () => {
  ...
  const changedPartitionCalls = new Map<string, number>();
  let previousPartitions: readonly EntityProjectionPartition[] | undefined;
  let projectorNoChangeCalls = 0;

  const projectorObject = (core as unknown as {
    entityProjector: { projectPartitions: (source: SimulationCore) => readonly EntityProjectionPartition[] };
  }).entityProjector;
  ...
  projectorObject.projectPartitions = (source: SimulationCore) => {
    ...
    try {
      const partitions = originalProjectPartitions(source);
      if (previousPartitions !== undefined) {
        let changed = 0;
        for (let index = 0; index < partitions.length; index++) {
          if (partitions[index] !== previousPartitions[index]) {
            changed++;
            const id = partitions[index]!.id;
            changedPartitionCalls.set(id, (changedPartitionCalls.get(id) ?? 0) + 1);
          }
        }
        if (changed === 0) projectorNoChangeCalls++;
      }
      previousPartitions = partitions;
      return partitions;
    } finally {
      projector.ms += performance.now() - start;
      projector.calls++;
    }
  };
  ...
  console.log('PHASE0B_VALIDATION_FASTPATH_PROFILE', JSON.stringify({
    totalMs: round(totalMs),
    syncMs: round(sync.ms), syncCalls: sync.calls,
    projectorMs: round(projector.ms), projectorCalls: projector.calls,
    projectorNoChangeCalls,
    changedPartitionCalls: Object.fromEntries([...changedPartitionCalls.entries()].sort()),

...
test("P2A fixture manifest covers the approved deterministic corpus", () => {
  ...
  const mutationNames = new Set(
    manifest.mutationCases.map((entry) => entry.name),
  );

  for (const name of REQUIRED_STATIC_CASES)
    assert.ok(staticNames.has(name), `missing static P2A fixture ${name}`);
  for (const name of REQUIRED_MUTATION_CASES)
    assert.ok(mutationNames.has(name), `missing mutation P2A fixture ${name}`);
  assert.ok(
    manifest.malformedCases.length >= 4,
    "malformed validation corpus must cover at least four failures",
  );
  ...
  try {
    for (const fixture of manifest.mutationCases) {
      ...
      assert.equal(
        native.status,
        0,
        `native parity CLI failed for ${fixture.name}: ${native.stderr}`,
      );
...
test("P2A parity fixture diagnostic capture", () => {
  ...
  console.log(
    `P2A_PARITY_FIXTURE_BASE64 ${Buffer.from(fixture, "utf8").toString("base64")}`,
  );
});
...
function runTypeScriptCase(
  fixture: P2AParityMutationCase,
): readonly ParityStep[] {
  ...
  for (const [index, command] of fixture.commands.entries()) {
    ...
    steps.push({
      ...
      parcelReferenceRewrites: Object.fromEntries(
        Object.entries(result.parcelReferenceRewrites).sort(([left], [right]) =>
          left.localeCompare(right),
        ),
      ),
      rejectionReasons: [...result.rejectionReasons],
      canonicalHash: prismCanonicalHashV1({
        world: fixture.envelope.world,
        cadastre: snapshot,
      }),
      totalParcelAreaM2: snapshot.parcels.reduce(
        (total, parcel) => total + parcel.areaM2,
        0,
      ),
      lineage,
      validation,
...
function applyCommand(
  system: CadastralMutationSystem,
  command: P2AMutationCommand,
  ...
  switch (command.kind) {
    ...
    case "create-easement":
      return system.createEasement(
        command.parcelIds,
        command.easementKind,
        command.geometry,
      );

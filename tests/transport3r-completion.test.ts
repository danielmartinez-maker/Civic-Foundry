...
import type { RoutingArc, RoutingState, RoutingTopology } from '../src/simulation/transportation/RoutingTopology.ts';
...
const ROUTING_TOPOLOGY = topology([
  { id: 'a-c1', fromStateKey: 'A|-', toState: { junctionId: 'B', incomingCarriagewayId: 'c1' }, carriagewayId: 'c1', laneGroupIds: ['lg1'], permissions: ALL_VEHICLE_PERMISSIONS, traversalTicks: 10, movementPenaltyTicks: 0 },
  { id: 'b-c2', fromStateKey: 'B|c1', toState: { junctionId: 'D', incomingCarriagewayId: 'c2' }, carriagewayId: 'c2', laneGroupIds: ['lg2'], movementId: 'm1', permissions: ALL_VEHICLE_PERMISSIONS, traversalTicks: 10, movementPenaltyTicks: 0 },
  { id: 'a-c3', fromStateKey: 'A|-', toState: { junctionId: 'C', incomingCarriagewayId: 'c3' }, carriagewayId: 'c3', laneGroupIds: ['lg3'], permissions: ALL_VEHICLE_PERMISSIONS, traversalTicks: 15, movementPenaltyTicks: 0 },
  { id: 'c-c4', fromStateKey: 'C|c3', toState: { junctionId: 'D', incomingCarriagewayId: 'c4' }, carriagewayId: 'c4', laneGroupIds: ['lg4'], movementId: 'm2', permissions: ALL_VEHICLE_PERMISSIONS, traversalTicks: 15, movementPenaltyTicks: 0 },
]);
...
test('dynamic routing reacts deterministically to congestion, restrictions, incidents, and destination accessibility', () => {
  ...
  const fast = routing.findRoute(ROUTING_TOPOLOGY, 'A', 'D', { permissions: ALL_VEHICLE_PERMISSIONS, destinationAccessible: true });
  assert.deepEqual(fast?.carriagewayIds, ['c1', 'c2']);
  const again = routing.findRoute(ROUTING_TOPOLOGY, 'A', 'D', { permissions: ALL_VEHICLE_PERMISSIONS, destinationAccessible: true });
  ...
  const diverted = routing.findRoute(ROUTING_TOPOLOGY, 'A', 'D', { permissions: ALL_VEHICLE_PERMISSIONS, destinationAccessible: true });
  assert.deepEqual(diverted?.carriagewayIds, ['c3', 'c4']);
  assert.equal(routing.findRoute(ROUTING_TOPOLOGY, 'A', 'D', { permissions: ALL_VEHICLE_PERMISSIONS, destinationAccessible: false }), null);
});
...
test('generalized travel cost produces an explainable causal breakdown and rejects non-finite inputs', () => {
  ...
  assert.throws(() => costs.evaluate({ ...result.input, inVehicleTimeTicks: Number.NaN }), /finite/);
});
...
test('transportation incidents own deterministic roadway effects without owning emergency dispatch', () => {
  ...
  assert.deepEqual(incidents.serviceRequests(), [{ incidentId: 'incident:1', requiredResponse: 'police' }]);
...
test('parking authority conserves capacity and exposes price, legality, availability, curb rules, and search impedance', () => {
  ...
  restored.restore(snapshot);
  assert.deepEqual(restored.snapshot(), snapshot);
  assert.throws(() => parking.upsert({ id: 'bad', destinationId: 'dest', capacity: 1, occupied: 2, legal: true, pricePerTrip: 0, baseSearchTicks: 0, curbRegulation: 'none' }), /occupancy/);
});

...
import { NativeEngineBridge, isNativeShadowEnabled, nativeShadowEnabledFromGlobal } from '../src/native/NativeEngineBridge.ts';
import { ShadowSimulationRunner } from '../src/native/ShadowSimulationRunner.ts';
import type { NativeEngineAddon, NativeEngineHandle } from '../src/native/NativeEngineTypes.ts';
...
function fakeAddon(): NativeEngineAddon & { calls: string[] } {
  ...
  return {
    calls,
    createEngine: () => { calls.push('create'); return handle; },
    destroyEngine: () => { calls.push('destroy'); },
    submitCommands: (_handle, json) => { calls.push(`submit:${json}`); },
    step: (_handle, ticks) => { calls.push(`step:${ticks}`); },
    loadV9: () => { calls.push('load'); },
    saveV9: () => '{"saveVersion":9}',
    getSnapshot: () => '{"hashVersion":1,"pendingCommands":[],"randomStreams":{},"seed":1,"speed":1,"tick":0}',
    getEvents: () => '[]',
    getDomainHash: (_handle: NativeEngineHandle, domain: string) => ({ ownership: domain === 'kernel' ? 1 : 2, version: 1, value: 42n }),
  };
}
...
test('shadow runner feeds identical normalized commands to both runtimes and ignores unowned domains', () => {
  ...
  runner.submit([{ sequence: 2, tick: 1, type: 'b', payload: null }, { sequence: 1, tick: 1, type: 'a', payload: null }]);
  runner.step(4);
  assert.deepEqual(received, [[1, 2]]);
  assert.deepEqual(steps, [4]);
  assert.deepEqual(runner.compareDomains(['kernel', 'world']).map((item) => ({ domain: item.domain, ownership: item.native.ownership, matches: item.matches })), [
...
test('native shadow mode is opt-in and disabled by default-like values', () => {
  ...
  assert.equal(nativeShadowEnabledFromGlobal({ __CIVIC_NATIVE_SHADOW__: '1' }), true);
});

...
import {
  HousingTenureSystem,
  type HousingTenureBuildingInput,
  type HousingTenureOption,
} from "../src/simulation/housing/HousingTenureSystem.ts";
...
test("housing tenure splits physical capacity by residential intensity", () => {
  ...
  const lowHome = low.byBuilding.home;
  assert.ok(lowHome);
  assert.equal(lowHome.totalCapacity, 100);
  assert.equal(lowHome.ownershipCapacity, 60);
  assert.equal(lowHome.rentalCapacity, 40);

  const medium = system.evaluate(0.045, [
    { ...baseInput, intensity: "medium" },
  ]);
  const mediumHome = medium.byBuilding.home;
  assert.ok(mediumHome);
  assert.equal(mediumHome.ownershipCapacity, 40);
  assert.equal(mediumHome.rentalCapacity, 60);

  const high = system.evaluate(0.045, [{ ...baseInput, intensity: "high" }]);
  const highHome = high.byBuilding.home;
  assert.ok(highHome);
  assert.equal(highHome.ownershipCapacity, 25);
  assert.equal(highHome.rentalCapacity, 75);
});
...
test("higher financing rates increase monthly owner burden under identical rent conditions", () => {
  ...
  const highRate = system.evaluate(0.1, [baseInput]).byBuilding.home!;
...
test("persistent housing initialization conserves population, respects capacity, and is input-order deterministic", async () => {
  ...
  for (const option of options) {
    const assigned = first
      ...
      .reduce(
        (sum: number, allocation: any) => sum + allocation.residents,
        0,
      );
...
test("displacing a building moves its actual cohorts into the displaced search queue and rehousing preserves resident mass", async () => {
  ...
  assert.equal(
    system
      .snapshotState()
      .unplaced.filter((cohort: any) => cohort.displaced)
      .reduce((sum: number, cohort: any) => sum + cohort.residents, 0),
    occupantsOfA,
  );
  ...
  assert.equal(
    system
      .snapshotState()
      .allocations.some(
        (allocation: any) => allocation.buildingId === "building:a",
      ),
    false,
  );
});
...
test("failed displaced search remains explicit unplaced state rather than disappearing", async () => {
  ...
  const snapshot = system.reconcile({
    population: 40,
    options: [],
    allowVoluntaryMoves: false,
  });
  ...
  assert.equal(
    state.unplaced.reduce(
      (sum: number, cohort: any) => sum + cohort.residents,
      0,
    ),
    40,
  );

...
function run(mode: 'no-sync' | 'sync-no-invariants' | 'normal'): number {
  ...
  if (mode === 'sync-no-invariants') (core.kernel.invariants as unknown as MutableInvariantRunner).runDue = () => {};
...
test('profile changed-partition commit cost separately from invariant cost', () => {
  const values: Record<'no-sync' | 'sync-no-invariants' | 'normal', number[]> = { 'no-sync': [], 'sync-no-invariants': [], normal: [] };
  const orders = [
    ['no-sync', 'sync-no-invariants', 'normal'],
    ['sync-no-invariants', 'normal', 'no-sync'],
    ['normal', 'no-sync', 'sync-no-invariants'],
  ] as const;
  for (const order of orders) for (const mode of order) values[mode].push(run(mode));

...
import {
  calculateLandPreparationMultiplier,
  SOIL_PROPERTIES,
} from "../src/world/terrain/SoilModel.ts";
...
function sample(
  patch: Partial<TerrainPhysicalSample> = {},
...
test("soil model exposes exactly eight locked engineering property sets", () => {
  ...
  for (const properties of Object.values(SOIL_PROPERTIES)) {
    assert.ok(
      Number.isFinite(properties.infiltrationMmPerHour) &&
        properties.infiltrationMmPerHour >= 0,
    );
    assert.ok(
      Number.isFinite(properties.bearingCapacityKpa) &&
        properties.bearingCapacityKpa > 0,
    );
    assert.ok(
      Number.isFinite(properties.erodibility) &&
        properties.erodibility >= 0 &&
        properties.erodibility <= 1,
    );
    assert.ok(
      Number.isFinite(properties.preparationBase) &&
        properties.preparationBase > 0,
    );
  }
});

test("weak wet peat costs more to prepare than strong drained gravel", () => {
  const gravel = calculateLandPreparationMultiplier({
    slope: 0.05,
    soilClass: "gravel",
    bedrockDepthMeters: 3,
    groundwaterDepthMeters: 8,
    contaminationIndex: 0,
    floodSusceptibility: 0,
  });
  const peat = calculateLandPreparationMultiplier({
    slope: 0.05,
    soilClass: "peat",
...
test("slope, shallow groundwater, contamination, deep bedrock, and flood susceptibility increase preparation cost directionally", () => {
  ...
  assert.ok(
    calculateLandPreparationMultiplier({ ...base, slope: 0.35 }) > baseline,
  );
  assert.ok(
    calculateLandPreparationMultiplier({
      ...base,
      groundwaterDepthMeters: 0.6,
    }) > baseline,
  );
  assert.ok(
    calculateLandPreparationMultiplier({ ...base, contaminationIndex: 0.8 }) >
      baseline,
  );
  assert.ok(
    calculateLandPreparationMultiplier({
  ...
  assert.ok(
    calculateLandPreparationMultiplier({ ...base, floodSusceptibility: 0.8 }) >
      baseline,
  );
});
...
test("TerrainField validates dimensions and physical values and returns isolated snapshots", () => {
  assert.throws(
    () => TerrainField.fromSamples(2, 2, 30, [sample()]),
    /sample count/,
  );
  assert.throws(
    () =>
      TerrainField.fromSamples(1, 1, 30, [
        sample({ elevationMeters: Number.NaN }),
      ]),
    /finite/,
  );
  const field = TerrainField.fromSamples(2, 1, 30, [
    sample({ landPreparationMultiplier: 1.2 }),
    sample({ elevationMeters: 121 }),
  ]);
  ...
  assert.deepEqual(
    TerrainField.restore(snapshot).snapshotAuthoritative(),
    snapshot,
  );
});

...
import { hydrateCoreV6, serializeCoreV5, serializeCoreV6 } from "../src/save/save.ts";
import { SimulationCore } from "../src/simulation/core/SimulationCore.ts";
import { TerrainGrid, type TerrainCell } from "../src/world/terrain/TerrainGrid.ts";

function flat(width = 40, height = 24) {
...
function buildCity(): SimulationCore {
  ...
  for (let x = 4; x <= 12; x++)
    core.paintZone([{ x, y: 11 }], "residential");
  for (let x = 22; x <= 27; x++)
    core.paintZone([{ x, y: 11 }], "commercial");
  for (let x = 28; x <= 34; x++)
    core.paintZone([{ x, y: 11 }], "industrial");
  for (const [x, y] of [
    [6, 13],
    [10, 13],
    [14, 13],
  ] as const)
    core.placeUtility("power", x, y);
  for (const [x, y] of [
    [18, 13],
    [22, 13],
    [26, 13],
  ] as const)
    core.placeUtility("water", x, y);
  for (const [x, y] of [
    [30, 13],
    [33, 13],
    [36, 13],
  ] as const)
    core.placeUtility("landfill", x, y);
  return core;
}
...
function advanceUntilFreight(core: SimulationCore, max = 3500): void {
  for (
    let i = 0;
    i < max && core.economyDomain.freightVehicles.activeCount() === 0;
    i++
  )
    core.step(1);
  assert.ok(
    core.economyDomain.freightVehicles.activeCount() > 0,
    "expected active freight before save",
  );
}
...
test("Save V6 resumes active firms freight and financial accruals identically", () => {
  const uninterrupted = buildCity();
  advanceUntilFreight(uninterrupted);
...
test("Save V6 rejects freight routes referencing missing road edges", () => {
  const core = buildCity();
  advanceUntilFreight(core);
  const save = structuredClone(serializeCoreV6(core));
  const [vehicle, ...remainingVehicles] = save.economyDomain.freightVehicles.vehicles;
  assert.ok(vehicle);
  const corrupted = {
    ...save,
    economyDomain: {
      ...save.economyDomain,
      freightVehicles: {
        ...save.economyDomain.freightVehicles,
        vehicles: [
          { ...vehicle, routeEdgeIds: ["missing-edge"] },
          ...remainingVehicles,
        ],
      },
    },
  };
  assert.throws(() => hydrateCoreV6(corrupted), /freight.*road|road.*freight/i);
});

...
function canonical<T extends { id: string }>(
  items: readonly T[],
): readonly T[] {
...
function normalizeTs(authority: TransportNetworkAuthority) {
  return {
    junctions: canonical(authority.junctions).map(({ id, x, y }) => ({
      id,
      x,
      y,
    })),
    segments: canonical(authority.segments).map(
      ({ id, startJunctionId, endJunctionId, carriagewayIds }) => ({
        id,
        startJunctionId,
        endJunctionId,
    ...
    carriageways: canonical(authority.carriageways).map(
      ({ id, segmentId, fromJunctionId, toJunctionId, laneIds }) => ({
        id,
        segmentId,
        fromJunctionId,
        toJunctionId,
    ...
    lanes: canonical(authority.lanes).map(
      ({ id, carriagewayId, ordinal, permissions, operatingState }) => ({
        id,
        carriagewayId,
        ordinal,
        permissions,
...
function normalizeNative(snapshot: NativeSnapshot) {
  return {
    ...
    segments: canonical(snapshot.segments).map((value) => ({
      ...value,
      carriagewayIds: [...value.carriagewayIds].sort(),
    })),
    carriageways: canonical(snapshot.carriageways).map((value) => ({
      ...value,
...
function firstDivergence(
  ...
  if (expected === null || actual === null)
    return `${path}: ${String(expected)} != ${String(actual)}`;
  if (Array.isArray(expected) || Array.isArray(actual)) {
    if (!Array.isArray(expected) || !Array.isArray(actual))
      return `${path}: array shape mismatch`;
  ...
  if (typeof expected === "object" && typeof actual === "object") {
    ...
    for (const key of keys) {
      if (!(key in left))
        return `${path}.${key}: missing from TypeScript projection`;
      if (!(key in right))
        return `${path}.${key}: missing from native projection`;
...
function assertParity(label: string, expected: unknown, actual: unknown): void {
  ...
  assert.equal(
    mismatch,
    undefined,
    `${label} first divergence: ${mismatch ?? "unknown"}`,
  );
}
...
function loadTs(cells: readonly RoadCell[], revision: number) {
  ...
  const authority = new LegacyRoadNetworkAdapter().projectAuthorityIfNeeded(
    roads,
  ).authority;
  const store = new TransportNetworkStore();
  assert.deepEqual(store.replaceAuthority(authority), {
    ok: true,
    changed: true,
  });
...
export function compareRoute(
  ...
  assert.ok(
    Number.isFinite(actual.totalCost),
    `${label}: native route cost must be finite`,
  );
  assert.ok(
    Math.abs(actual.totalCost - expected.totalCost) <= 1e-6,
    `${label}: route cost ${actual.totalCost} != ${expected.totalCost}`,
  );
}

export type ShadowRoadCell = RoadCell;

export function assertNetworkParity(
  handle: number,
  label: string,
  cells: readonly RoadCell[],
  revision: number,

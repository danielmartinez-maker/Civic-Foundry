...
test("deterministic diagnostics canonicalize object keys and reject illegal numeric state", () => {
  ...
  assert.notEqual(
    deterministicHash(left),
    deterministicHash({ ...right, z: 3 }),
  );
  assert.throws(
    () => stableStringify({ bad: Number.NaN }),
    /non-finite number at \$\.bad/,
  );
  assert.throws(
...
test("numeric guards report deterministic ownership paths without clamping", () => {
  ...
  assert.throws(
    () => assertFiniteNumber(Number.NaN, "traffic.weight"),
    /traffic\.weight must be finite/,
  );
  assert.throws(
    () =>
      assertFiniteRecord(
        { speed: 30, nested: { cost: Number.NEGATIVE_INFINITY } },
        "route",
      ),
...
test("transaction coordinator snapshots in stable id order and rolls back in reverse order", () => {
  ...
  coordinator.register({
    id: "b",
    snapshot: () => {
      trace.push("snapshot:b");
      return b.value;
    },
    restore: (value) => {
      trace.push("restore:b");
      b.value = value;
    },
  });
  coordinator.register({
    id: "a",
    snapshot: () => {
      trace.push("snapshot:a");
      return a.value;
    },
    restore: (value) => {
      trace.push("restore:a");
      a.value = value;
    },
  });
  ...
  assert.throws(
    () =>
      coordinator.register({ id: "a", snapshot: () => 0, restore: () => {} }),
    /duplicate transaction participant: a/,
  );
});
...
test("transaction rollback failure is fail-stop structured architecture failure", () => {
  ...
  assert.throws(
    ...
    (error: unknown) =>
      ...
      error.category === "TransactionFailure",
  );
});

test("causal trace is deterministic, bounded, and preserves parent-child relations", () => {
  const trace = new CausalTraceBuffer(3);
  const root = trace.append({
    code: "route-invalidated",
    domain: "transport",
    operation: "route",
    tick: 9,
    entityIds: ["trip-1"],
  });
  const child = trace.append({
    code: "vehicle-rerouted",
    domain: "transport",
    operation: "reroute",
    tick: 9,
  ...
  trace.append({
    ...
    operation: "complete",
    tick: 10,
    parentSequence: child.sequence,
  });

  assert.deepEqual(
    trace.list().map((item) => item.code),
    ["vehicle-rerouted", "cache-rebuild", "trip-complete"],
  );
...
test("reference integrity validator reports duplicate, dangling, stale and non-finite seams", () => {
  ...
  assert.deepEqual(
    validator.failures().map((failure) => failure.code),
    [
      "duplicate-entity-id",
      "dangling-reference",
  ...
  assert.throws(
    () => validator.throwIfAny("save-v9"),
    (error: unknown) =>
      error instanceof EngineFailure &&
      error.code === "reference-integrity-failed",
  );
});
...
test("repro bundles serialize deterministically and replay the same failure code and pre-failure hash", () => {
  ...
  const result = replayReproBundle(bundle, () => ({
    failureCode: "forced-test-failure",
    preFailureAuthorityHash: "bb22",
  }));
  ...
  assert.throws(
    () =>
      replayReproBundle(bundle, () => ({
        failureCode: "different",
        preFailureAuthorityHash: "bb22",
      })),
    (error: unknown) =>
      error instanceof EngineFailure &&
      error.code === "repro-failure-code-mismatch",
  );
});
...
test("kernel exposes stable scheduler contracts, structured failure state and performance diagnostics", () => {
  ...
  const failing = new SimulationKernel({
    clock: new SimulationClock(),
    seed: 1,
    now: () => 0,
  });
  failing.registerSystem({
    id: "boom",
    reads: [],
    writes: ["state"],
    cadence: { every: 1 },

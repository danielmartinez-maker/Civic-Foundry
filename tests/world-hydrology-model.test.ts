...
function funnelTerrain(width = 7, height = 7): { terrain: TerrainField; conditioned: Float64Array } {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      samples.push({
        elevationMeters: e, slope: 0.05, aspectRadians: 0, soilClass: 'loam', soilDepthMeters: 2,
        bearingCapacityKpa: 160, bedrockDepthMeters: 5, groundwaterDepthMeters: 3,
        vegetationClass: 'grass', contaminationIndex: 0, landPreparationMultiplier: 1,
        surfaceWater: 'none', buildable: true,
      });
    }
  }
  return { terrain: TerrainField.fromSamples(width, height, 30, samples), conditioned: elevation };
}
...
test('hydrology model assigns every cell to one watershed and conserves accumulation downstream', () => {
  ...
  assert.equal(snapshot.watershedIds.every((id) => id.length > 0), true);
  assert.equal(snapshot.watersheds.reduce((sum, item) => sum + item.memberCount, 0), 49);
  for (let index = 0; index < snapshot.receiver.length; index++) {
    const receiver = snapshot.receiver[index];
    if (receiver !== null) assert.ok(snapshot.flowAccumulation[receiver]! >= snapshot.flowAccumulation[index]!);
  }
});
...
test('terrain-derived channels follow drainage receivers and include the high-accumulation trunk', () => {
  ...
  for (const channel of snapshot.channels) assert.equal(channel.toIndex, snapshot.receiver[channel.fromIndex]);
...
test('static flood susceptibility is higher in the convergent lowland than on a ridge cell', () => {
  ...
  assert.ok(lowland.floodSusceptibility >= 0 && lowland.floodSusceptibility <= 1);
});

test('hydrology snapshot restores exactly', () => {
  const { terrain, conditioned } = funnelTerrain();
  const snapshot = HydrologyModel.build(terrain, conditioned).snapshotAuthoritative();

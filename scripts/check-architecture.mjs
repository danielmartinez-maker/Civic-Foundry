...
const rules = [
  { importer: 'src/simulation/', forbidden: 'src/app/', rule: 'simulation-no-app' },
  { importer: 'src/simulation/', forbidden: 'src/ui/', rule: 'simulation-no-ui' },
  { importer: 'src/simulation/', forbidden: 'src/rendering/', rule: 'simulation-no-rendering' },
  { importer: 'src/world/', forbidden: 'src/app/', rule: 'world-no-app' },
  { importer: 'src/world/', forbidden: 'src/ui/', rule: 'world-no-ui' },
  { importer: 'src/world/', forbidden: 'src/rendering/', rule: 'world-no-rendering' },
  ...
  { importer: 'src/rendering/', forbidden: 'src/app/', rule: 'rendering-no-app' },
...
export async function runArchitectureCheck() {
  ...
  if (failures.length > 0) {
    ...
    for (const failure of failures) {
      console.error(`${failure.importer} -> ${failure.imported} violates ${failure.rule}`);
    }
    process.exitCode = 1;
    return;
  }

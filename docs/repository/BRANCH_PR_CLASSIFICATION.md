...
There were 22 open pull requests before Stack 7 opened PR #115. Stack 7 therefore observed 23 open PRs during execution.

| Branch / PR | Purpose | Canonical stack / phase | Base | Head | Integrated? | CI | Classification | Required action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| #115 | Stack 7 repository health | Stack 7 | `main@6e1b9870` | `chore/stack-7-repository-health-hygiene` | No | Exact-head gate required | Active implementation | Keep draft until acceptance and merge authorization |
| #114 | 3D presentation and asset scale-up | Stack 3 | `design/3d-presentation-asset-program` | `feature/stack-3-3d-presentation-asset-scaleup` | No | Active branch CI | Active implementation | Preserve draft; do not merge without authorization |
...
| #103 | Specialized GPU overlays | Older presentation program | `feature/gpu-parity-retained-scene` | `feature/gpu-specialized-overlay-parity` | No | Historical branch CI | Accepted baseline required by active stack | Preserve as Stack 3 semantic donor until donor review completes |
| #99 | Retained GPU scene | Older presentation program | `main@815b1548` | `feature/gpu-parity-retained-scene@3b4e08a8` | No | Run `33140360165` reported green | Accepted baseline required by active stack | Preserve as Stack 3 semantic donor |
| #97 | Disposable main→Phase 0B sync | Phase 0B | `civic-2.0-phase-0b-forward-port` | `sync/main-into-phase-0b-forward-port` | No | Not acceptance-relevant | Superseded | Close; branch preserved pending owner cleanup |
| #96 | Isometric B2 public realm | Older presentation program | `feature/isometric-pass-b1-urban-depth` | `feature/isometric-pass-b2-public-realm@67bbf1bd` | No | Run `33029642106` reported green | Accepted baseline required by active stack | Preserve as Stack 3 semantic donor |
| #91 | Isometric B1 urban depth | Older presentation program | `main@ee296a98` | `feature/isometric-pass-b1-urban-depth@e1021ad4` | No | PR reports exact-head green | Accepted baseline required by active stack | Preserve as Stack 3 semantic donor |
...
| #88 | 3R-B intersection control | Transportation 3R | `main@ee296a98` | `civic-2.0-3r-b-intersection-control` | No | Design/planning only | Active design | Reconcile stale Save assumptions before implementation; do not merge |
| #77 | Disposable patch runner | Phase 0B | `phase0b-perf-base-anchor` | `phase0b-outer-noop-patch-runner` | No | Utility runner | Superseded | Close; branch preserved pending owner cleanup |
| #75 | Disposable profiler | Phase 0B | `civic-2.0-phase-0b-inline` | `phase0b-validation-fastpath-profile` | No | Profiling-only | Superseded | Close; branch preserved pending owner cleanup |
...
| #59 | Disposable GREEN gate | Phase 0B | `phase0b-perf-base-anchor` | `phase0b-incremental-integrity-green-gate` | No | Verification-only | Superseded | Close; branch preserved pending owner cleanup |
...
| #16 | Temporary Phase 0A browser smoke | Phase 0A | `civic-2.0-phase-0a-inline` | `verify-phase0a-browser-smoke` | No | Verification-only | Superseded | Close; branch preserved pending owner cleanup |
...
The baseline inventory found 176 remote branches including `main`; therefore 175 non-`main` branches require a classification. The exact branch-name inventory and family membership are recorded in `docs/repository/STACK_7_HEALTH_BASELINE.md`. Those groups map one-to-one to the required Stack 7 categories as follows:
...
| Merged and candidate for safe deletion after merge/dependency proof | **6. Merged and safely deletable** only after the final reference checks are completed | Do not delete from this tranche without proof |
...
No remote branch is deleted by Stack 7. The GitHub connection available in this execution can update source/PR state but does not provide a safe branch-delete workflow with the dependency/history checks required by the Stack 7 destructive-branch rule. More importantly, several active stacked PRs still name historical presentation and Prism refs as bases or semantic donors.

...
std::expected<void,std::string> NativeRenderer::render(const SceneGeometry& geometry,const FrameToken& frame){if(!initialized_)if(auto result=initialize();!result)return result;if(auto uploaded=uploadIfChanged(geometry);!uploaded)return uploaded;if(!geometry.opaque.empty()){if(geometry.opaque.size()>std::numeric_limits<std::uint32_t>::max())return std::unexpected("opaque scene exceeds draw vertex limit");DrawCommand draw{opaque_pipeline_,opaque_buffer_,static_cast<std::uint32_t>(geometry.opaque.size()),static_cast<std::uint32_t>(sizeof(SceneVertex)),0};if(auto recorded=backend_.recordDraw(frame,draw);!recorded)return recorded;++stats_.draw_calls;}if(!geometry.overlay.empty()){if(geometry.overlay.size()>std::numeric_limits<std::uint32_t>::max())return std::unexpected("overlay scene exceeds draw vertex limit");DrawCommand draw{overlay_pipeline_,overlay_buffer_,static_cast<std::uint32_t>(geometry.overlay.size()),static_cast<std::uint32_t>(sizeof(SceneVertex)),0};if(auto recorded=backend_.recordDraw(frame,draw);!recorded)return recorded;++stats_.draw_calls;}return {};}

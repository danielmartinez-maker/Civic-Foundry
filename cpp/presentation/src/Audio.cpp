...
AudioMix AudioPlanner::plan(const FrameSnapshot& snapshot, const PresentationSettings& requested) const noexcept {
    ...
    for (const auto& vehicle : snapshot.vehicles) {
        ...
        if (vehicle.kind == VehicleKind::Bus || vehicle.kind == VehicleKind::Brt || vehicle.kind == VehicleKind::Tram || vehicle.kind == VehicleKind::Metro || vehicle.kind == VehicleKind::Rail) ++transit;
        if (vehicle.kind == VehicleKind::Emergency) ++emergency;
    }
    std::size_t construction = 0;
    for (const auto& building : snapshot.buildings) if (building.construction_progress < 0.999F) ++construction;
    float water = 0.0F;
    for (const auto& terrain : snapshot.terrain) if (terrain.water || terrain.flood_depth_m > 0.01F) water += 1.0F;

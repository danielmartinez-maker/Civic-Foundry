...
Result<void> NativeEngine::applyReadyCommands() {
    ...
    for (const auto& command : ready) {
        auto transportationApplied = applyTransportationCommand(transportation_, legacy_roads_, command);
        if (!transportationApplied) { rollback(); return std::unexpected(transportationApplied.error()); }
        const bool socioeconomicTransfer = command.type.starts_with(socioeconomic_transfer_prefix);
        auto socioeconomicApplied = applySocioeconomicBridgeCommand(command);
        if (!socioeconomicApplied) { rollback(); return socioeconomicApplied; }
        const char* source = socioeconomicTransfer
            ? "native-authority-transfer"
            : (*transportationApplied ? "transportation-native" : "shadow-command");
        events_.append(clock_.tick(), command.type, source, command.payload);
    }
    return {};
}
...
Result<std::string> NativeEngine::saveV9() const {
    ...
    auto transportationSave = writeNativeEngineV9(
        loaded_save_->canonicalJson,
        legacy_roads_,
        transportation_.snapshot(),
        clock_.tick(),
        static_cast<std::uint32_t>(clock_.speed()));
    if (!transportationSave) return std::unexpected(transportationSave.error());

    auto root = parseJsonObject(*transportationSave);

...
cf_error_code cf_engine_load_v9(cf_engine* engine, const uint8_t* data, size_t size) { return guarded(engine,[&]()->civic::Result<void>{ if(!data){ if(size!=0)return std::unexpected(civic::make_error(civic::ErrorCode::invalid_argument,"save buffer is null")); return std::unexpected(civic::make_error(civic::ErrorCode::serialization_failure,"save buffer is empty")); } return engine->value->loadV9Authoritative(std::string_view(reinterpret_cast<const char*>(data),size)); }); }
cf_error_code cf_engine_stage_save_v9(cf_engine* engine, const uint8_t* data, size_t size) { return guarded(engine,[&]()->civic::Result<void>{ if(!data){ if(size!=0)return std::unexpected(civic::make_error(civic::ErrorCode::invalid_argument,"save buffer is null")); return std::unexpected(civic::make_error(civic::ErrorCode::serialization_failure,"save buffer is empty")); } return engine->value->stageSaveV9Envelope(std::string_view(reinterpret_cast<const char*>(data),size)); }); }
cf_error_code cf_engine_save_v9(cf_engine* engine, cf_buffer* out_buffer) { return guarded(engine,[&]()->civic::Result<void>{ auto result=engine->value->saveV9Authoritative(); if(!result)return std::unexpected(result.error()); const auto copied=copyBuffer(*result,out_buffer); if(copied!=CF_ERROR_NONE)return std::unexpected(civic::make_error(civic::ErrorCode::internal_error,"failed to allocate save buffer")); return {}; }); }

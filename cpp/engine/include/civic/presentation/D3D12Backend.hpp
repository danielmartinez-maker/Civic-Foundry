...
public:
    ...
    std::expected<void, std::string> initialize(void* native_window, std::uint32_t width, std::uint32_t height) override;
    std::expected<void, std::string> resize(std::uint32_t width, std::uint32_t height) override;
    std::expected<BufferHandle, std::string> createBuffer(const BufferDesc&, std::span<const std::byte> initial_data) override;
    ...
    std::expected<void, std::string> waitForFence(std::uint64_t value) override;

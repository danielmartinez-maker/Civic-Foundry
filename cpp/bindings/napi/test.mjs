...
addon.submitCommands(left, JSON.stringify([{ sequence: 1, tick: 1, type: 'semantic', payload: { a: 1, b: 2 } }]));
addon.submitCommands(right, JSON.stringify([{ sequence: 1, tick: 1, type: 'semantic', payload: { b: 2, a: 1 } }]));
assert.equal(addon.getDomainHash(left, 'kernel').value, addon.getDomainHash(right, 'kernel').value);

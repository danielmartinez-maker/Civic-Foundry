...
fn component_value_requires_exact_registered_width() {
    ...
    let value = ComponentValue::new(layout.type_id(), vec![1, 2, 3, 4], &layout)
...
fn component_registry_is_idempotent_but_rejects_conflicts() {
    ...
    .expect("valid layout");
    let conflicting = ComponentLayout::new(
        ComponentTypeId::new(9),
        4,
        4,
    ...
    registry.register(first).expect("identical registration is idempotent");

...
import {
  ...
  type PassengerQueueSnapshot,
} from "../transit/PassengerQueueSystem.ts";
import {
  TransitVehicleSystem,
  type TransitVehicleStateSnapshot,
} from "../transit/TransitVehicleSystem.ts";
import {
  TransitOperationsSystem,
  type TransitOperationsStateSnapshot,
} from "../transit/TransitOperationsSystem.ts";
...
export type MobilityFiscalDelta = Readonly<{
...
const MAX_ACCEPTABLE: Readonly<Record<MobilityPersonTrip["purpose"], number>> =
  Object.freeze({ commute: 240, shopping: 180 });
...
export class MobilityScheduler {
  ...
  tick(context: MobilityTickContext): MobilitySnapshot {
    this.multimodalGraph.rebuild(
      context.roadGraph,
      context.transit,
      (lineId, fromStopId, toStopId, mode) =>
        this.segmentTicks(lineId, fromStopId, toStopId, mode, context),
  ...
  snapshot(): MobilitySnapshot {
    const totalDecisionWeight = this.decisions.reduce(
      (sum, decision) => sum + decision.travelerWeight,
      0,
    );
    const modeWeight = (mode: MobilityDecision["mode"]): number =>
      this.decisions
        .filter((decision) => decision.mode === mode)
        .reduce((sum, decision) => sum + decision.travelerWeight, 0);
    const successful = this.decisions.filter(
      (decision) =>
        decision.mode !== "unmet" && Number.isFinite(decision.chosenCost),
    );
    const successfulWeight = successful.reduce(
      (sum, decision) => sum + decision.travelerWeight,
      0,
    );
    ...
    const transitDecisions = this.decisions.filter(
      (decision) => decision.mode === "transit",
    );
    const transitWeight = transitDecisions.reduce(
      (sum, decision) => sum + decision.travelerWeight,
      0,
    );
    const scheduledWaitTicks =
      transitWeight <= 0
        ? 0
        : transitDecisions.reduce(
            (sum, decision) =>
              sum + decision.expectedWaitTicks * decision.travelerWeight,
            0,
          ) / transitWeight;
    ...
    for (const lineId of lineIds) {
      const line = this.operations.snapshotLineWithVehicles(
        lineId,
        this.vehicles,
      );
    ...
    const onboard = this.vehicles
      .listVehicles()
      .reduce(
        (sum, vehicle) =>
          sum +
          vehicle.onboard.reduce(
            (inner, cohort) => inner + cohort.travelerWeight,
            0,
          ),
        0,
      );
    const totalCapacity = this.vehicles
      .listVehicles()
      .reduce((sum, vehicle) => sum + vehicle.capacity, 0);
    const crowding =
      totalCapacity <= 0
        ? 0
        : Math.max(0, Math.min(1, onboard / totalCapacity));

    return Object.freeze({
      carModeShare:
        totalDecisionWeight <= 0 ? 0 : modeWeight("car") / totalDecisionWeight,
      transitModeShare:
        totalDecisionWeight <= 0
          ? 0
          : modeWeight("transit") / totalDecisionWeight,
      unmetShare:
        totalDecisionWeight <= 0
          ? 0
          : modeWeight("unmet") / totalDecisionWeight,
      personAccessibility:
        totalDecisionWeight <= 0
          ? 1
          : successfulWeight <= 0
            ? 0
            : weightedAccessibility / totalDecisionWeight,
      ridership,
      meanWaitTicks,
      reliability:
        reliabilityWeight <= 0 ? 1 : reliabilityNumerator / reliabilityWeight,
      crowding,
  ...
  consumeFiscalDelta(): MobilityFiscalDelta {
    ...
    const operatingCost = Math.max(
      0,
      snapshot.transitOperatingCost - this.fiscalOperatingCursor,
    );
    const fareRevenue = Math.max(
      0,
      snapshot.transitFareRevenue - this.fiscalFareCursor,
    );
  ...
  snapshotState(): MobilitySchedulerStateSnapshot {
    return Object.freeze({
      decisions: Object.freeze(
        this.decisions.map((decision) => Object.freeze({ ...decision })),
      ),
  ...
  restoreState(state: MobilitySchedulerStateSnapshot): void {
    if (
      !Number.isFinite(state.crowdingPenaltyTicks) ||
      state.crowdingPenaltyTicks < 0 ||
      !Number.isFinite(state.fiscalOperatingCursor) ||
      state.fiscalOperatingCursor < 0 ||
      !Number.isFinite(state.fiscalFareCursor) ||
      state.fiscalFareCursor < 0
    ) {
      throw new Error("invalid mobility scheduler state");
    }
    this.decisions.length = 0;
    this.decisions.push(
      ...state.decisions.map((decision) => Object.freeze({ ...decision })),
    );
  ...
  private routeTrip(
    ...
    const rawTransitPlan = this.journeyPlanner.plan(
      this.multimodalGraph,
      start,
      end,
      {
        mode: "transit",
    ...
    const transitPlan = this.applyGeneralizedCost(
      "transit",
      trip,
      rawTransitPlan,
      context,
    ...
    const choice = this.modeChoice.choose(carPlan, transitPlan, {
      crowdingPenaltyTicks:
        this.crowdingPenaltyTicks + this.capacityPressureTicks(),
    });
    ...
    if (
      ...
      carRoute &&
      carPlan &&
      (context.reserveCarTrip?.(trip) ?? true)
    ) {
    ...
    if (
      choice.mode === "transit" &&
      transitPlan &&
      this.enqueueTransitTrip(trip, transitPlan, context.transit)
    ) {
      this.record({
        mode: "transit",
        travelerWeight: trip.travelerWeight,
        purpose: trip.purpose,
        chosenCost: choice.transitCost,
        expectedWaitTicks: transitPlan.expectedWaitTicks,
      });
      return;
    }
    if (
  ...
  private applyGeneralizedCost(
    ...
    if (!Number.isFinite(cost) || cost < 0)
      throw new Error("invalid generalized travel cost");
  ...
  private enqueueTransitTrip(
    trip: MobilityPersonTrip,
    plan: JourneyPlan,
    transit: TransitNetworkSystem,
  ): boolean {
    const boardingLegs = plan.legs.filter(
      (leg) => leg.kind === "board" && leg.lineId,
    );
    const alightLegs = plan.legs.filter(
      (leg) => leg.kind === "alight" && leg.lineId,
    );
    if (boardingLegs.length === 0 || boardingLegs.length !== alightLegs.length)
      return false;
    ...
    for (let i = 1; i < boardingLegs.length; i++) {
      ...
      if (!board?.lineId || !alight?.lineId || board.lineId !== alight.lineId)
        return false;
      transfers.push({
        lineId: board.lineId,
        directionKey: this.directionForPlan(
          plan,
          transit,
          board.lineId,
          board.to,
    ...
    return this.passengers.enqueue(
      cohort.boardingStopId,
      cohort.lineId,
      cohort.directionKey,
      cohort,
    );
  ...
  private directionForPlan(
    ...
    const ride = plan.legs.find(
      (leg) =>
        leg.kind === "ride" &&
        leg.lineId === lineId &&
        leg.from === platformFrom,
    );
  ...
  private segmentTicks(
    ...
    if (mode === "metro")
      return Math.max(
        10,
        (Math.abs(from.x - to.x) + Math.abs(from.y - to.y)) * 5,
      );
    ...
    for (const start of starts) {
      for (const end of ends) {
        const candidate = context.pathfinding.findRoute(
          context.roadGraph,
          start,
          end,
          {
        ...
        if (
          !route ||
          candidate.totalCost < route.totalCost - 1e-9 ||
          (Math.abs(candidate.totalCost - route.totalCost) <= 1e-9 &&
            candidate.edgeIds.join("|").localeCompare(route.edgeIds.join("|")) <
              0)
        )
          route = candidate;
      }
    ...
    const free = route.edgeIds.reduce(
      (sum, edgeId) =>
        sum + (context.roadGraph.getEdge(edgeId)?.freeFlowTicks ?? 0),
      0,
    );
  ...
  private capacityPressureTicks(): number {
    ...
    for (const queue of this.passengers.snapshot().queues) {
      const waiting = queue.cohorts.reduce(
        (sum, cohort) => sum + Math.max(0, cohort.travelerWeight),
        0,
      );
      if (waiting > 0)
        waitingByLine.set(
          queue.lineId,
          (waitingByLine.get(queue.lineId) ?? 0) + waiting,
        );
    }
    ...
    for (const vehicle of this.vehicles.listVehicles()) {
      ...
      capacityByLine.set(
        vehicle.lineId,
        (capacityByLine.get(vehicle.lineId) ?? 0) +
          Math.max(0, vehicle.capacity),
      );
    }

    let totalWaiting = 0;
    let weightedPenalty = 0;
    for (const [lineId, waiting] of [...waitingByLine.entries()].sort((a, b) =>
      ...
      const penalty =
        activeCapacity <= 0
          ...
          : Math.min(
              600,
              (waiting / Math.max(1, activeCapacity)) *
                QUEUE_PRESSURE_TICKS_PER_VEHICLE_LOAD,
            );

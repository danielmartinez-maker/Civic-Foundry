...
export type MovementPermission =
  "protected" | "permissive" | "stop" | "prohibited";
...
export type IntersectionControlSnapshot = Readonly<{
  ...
  serviceCredits: readonly Readonly<{
    movementId: TurnMovementId;
    credit: number;
  }>[];
  lastSteppedTicks: readonly Readonly<{
    junctionId: JunctionId;
    tick: number;
  }>[];
}>;
...
function entryOrder(
  a: MutableMovementQueueEntry,
  b: MutableMovementQueueEntry,
): number {
  const priority =
...
function axisForApproach(
  ...
  const origin = incoming
    ? junctionById.get(incoming.fromJunctionId)
    : undefined;
  if (!incoming || !junction || !origin) return "horizontal";
  return Math.abs(origin.x - junction.x) >= Math.abs(origin.y - junction.y)
    ? "horizontal"
    : "vertical";
}
...
export class IntersectionControlSystem {
  ...
  private readonly controlOverrides = new Map<
    JunctionId,
    IntersectionControlType
  >();
  private readonly queues = new Map<
    TurnMovementId,
    MutableMovementQueueEntry[]
  >();
  private readonly pendingReleased = new Map<
    JunctionId,
    Map<string, TurnMovementId>
  >();
  ...
  configure(authority: TransportNetworkAuthority): void {
    ...
    this.junctionById = new Map(
      authority.junctions.map((junction) => [junction.id, junction]),
    );
    this.carriagewayById = new Map(
      authority.carriageways.map((carriageway) => [
        carriageway.id,
        carriageway,
      ]),
    );
    this.segmentById = new Map(
      authority.segments.map((segment) => [segment.id, segment]),
    );
    this.movementById = new Map(
      authority.movements.map((movement) => [movement.id, movement]),
    );
    ...
    for (const movementId of [...this.serviceCredits.keys()]) {
      if (!this.movementById.has(movementId))
        this.serviceCredits.delete(movementId);
    }
    for (const junctionId of [...this.controlOverrides.keys()]) {
      if (!this.junctionById.has(junctionId))
        this.controlOverrides.delete(junctionId);
    }
    for (const [junctionId, releases] of [...this.pendingReleased.entries()]) {
      ...
      if (releases.size === 0 || !this.junctionById.has(junctionId))
        this.pendingReleased.delete(junctionId);
    }
  }
  ...
  setControlType(
    ...
    if (!this.junctionById.has(junctionId))
      throw new Error(`unknown junction ${junctionId}`);
  ...
  controlType(junctionId: JunctionId): IntersectionControlType {
    ...
    const incoming = this.authority.carriageways.filter(
      (carriageway) => carriageway.toJunctionId === junctionId,
    );
    const signalWarrant = incoming.some(
      (carriageway) =>
        roadClassRank(carriageway.operatingClass) >= roadClassRank("arterial"),
    );
  ...
  movementPermission(
    ...
    const phase =
      ((Math.floor(tick) % SIGNAL_CYCLE_TICKS) + SIGNAL_CYCLE_TICKS) %
      SIGNAL_CYCLE_TICKS;
    const axis = axisForApproach(
      movement,
      this.junctionById,
      this.carriagewayById,
    );
    const throughGreen =
      axis === "horizontal"
    ...
    if (movement.turnKind === "left")
      return protectedLeft ? "protected" : "prohibited";
    if (movement.turnKind === "right")
      return throughGreen ? "protected" : "permissive";
  ...
  enqueue(movementId: TurnMovementId, entry: MovementQueueEntry): void {
    const movement = this.requireMovement(movementId);
    if (!movement.allowed)
      throw new Error(`movement ${movementId} is not allowed`);
    if (!entry.vehicleId) throw new Error("vehicleId must not be empty");
    if (!Number.isFinite(entry.travelerWeight) || entry.travelerWeight <= 0)
  ...
  queueDemand(movementId: TurnMovementId): number {
    return (this.queues.get(movementId) ?? []).reduce(
      (sum, entry) => sum + entry.travelerWeight,
      0,
    );
  }
  ...
  queueLength(junctionId?: JunctionId): number {
    ...
    for (const [movementId, entries] of this.queues) {
      if (
        ...
        this.movementById.get(movementId)?.junctionId !== junctionId
      )
        continue;
  ...
  stepJunction(
    junctionId: JunctionId,
    tick: number,
    context: IntersectionCapacityContext = {},
  ): string[] {
    ...
    const elapsedTicks =
      previousTick === undefined ? 1 : Math.max(1, tick - previousTick);
    ...
    const movementIds = this.authority.movements
      .filter(
        (movement) =>
          ...
          movement.allowed &&
          (this.queues.get(movement.id)?.length ?? 0) > 0,
      )
    ...
    for (const movementId of movementIds) {
      ...
      this.serviceCredits.set(
        movementId,
        Math.min(cap, credit + perTick * elapsedTicks),
      );
    }

    const control = this.controlType(junctionId);
    const protectedIds = movementIds.filter(
      (movementId) => this.movementPermission(movementId, tick) === "protected",
    );
    const permissiveIds = movementIds.filter(
      (movementId) =>
        this.movementPermission(movementId, tick) === "permissive",
    );
    const stopIds = movementIds.filter(
      (movementId) => this.movementPermission(movementId, tick) === "stop",
    );
    ...
    if (control === "stop") {
      ...
      for (const movementId of protectedIds)
        releases.push(...this.dischargeMovement(movementId));
    ...
    this.pendingReleased.set(
      junctionId,
      new Map(
        releases.map((release) => [release.vehicleId, release.movementId]),
      ),
    );
  ...
  snapshot(): IntersectionControlSnapshot {
    return Object.freeze({
      controls: Object.freeze(
        [...this.controlOverrides.entries()]
          .sort(([a], [b]) => a.localeCompare(b))
          .map(([junctionId, controlType]) =>
            Object.freeze({ junctionId, controlType }),
          ),
      ),
          ...
          .map(([junctionId, releases]) =>
            Object.freeze({
              ...
              releases: Object.freeze(
                [...releases.entries()]
                  .sort(([a], [b]) => a.localeCompare(b))
                  .map(([vehicleId, movementId]) =>
                    Object.freeze({ vehicleId, movementId }),
                  ),
              ),
            }),
          ),
  ...
  restore(snapshot: IntersectionControlSnapshot): void {
    ...
    for (const item of snapshot.controls)
      this.setControlType(item.junctionId, item.controlType);
    ...
    for (const item of snapshot.pendingReleased) {
      if (!this.junctionById.has(item.junctionId))
        throw new Error(`unknown junction ${item.junctionId}`);
      ...
      for (const release of item.releases) {
        ...
        if (movement.junctionId !== item.junctionId)
          throw new Error("released movement belongs to the wrong junction");
        if (this.hasVehicle(release.vehicleId))
          throw new Error(
            `duplicate intersection vehicle ${release.vehicleId}`,
          );
        releases.set(release.vehicleId, release.movementId);
      }
      if (releases.size > 0)
        this.pendingReleased.set(item.junctionId, releases);
    }
  ...
  private hasVehicle(vehicleId: string): boolean {
    for (const entries of this.queues.values())
      if (entries.some((entry) => entry.vehicleId === vehicleId)) return true;
    for (const releases of this.pendingReleased.values())
      if (releases.has(vehicleId)) return true;
    return false;
  }

  private selectHeadMovement(
    movementIds: readonly TurnMovementId[],
  ): TurnMovementId | undefined {
    return movementIds
      .map((movementId) => ({
        movementId,
        entry: this.queues.get(movementId)?.[0],
      }))
      .filter(
        (
          candidate,
          ...
          movementId: TurnMovementId;
          entry: MutableMovementQueueEntry;
  ...
  private serviceCapacityPerTick(
    movementId: TurnMovementId,
    context: IntersectionCapacityContext,
  ): number {
    const movement = this.requireMovement(movementId);
    ...
    const multiplierRaw = segment
      ? (context.capacityMultiplierBySegment?.[segment.id] ?? 1)
      : 1;
    const multiplier = Number.isFinite(multiplierRaw)
      ? Math.max(0, Math.min(1, multiplierRaw))
      : 0;

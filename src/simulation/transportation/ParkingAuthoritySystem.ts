...
function canonicalFacility(input: ParkingFacility): ParkingFacility {
  if (input.id.length === 0) throw new Error("parking facility id must be non-empty");
  if (input.destinationId.length === 0) throw new Error("parking destinationId must be non-empty");
  const capacity = safeCount(input.capacity, "parking capacity");
  const occupied = safeCount(input.occupied, "parking occupancy");
  if (occupied > capacity) throw new Error("parking occupancy cannot exceed capacity");
  return Object.freeze({
    ...input,
    capacity,
    occupied,
    pricePerTrip: finiteNonNegative(input.pricePerTrip, "parking pricePerTrip"),
    baseSearchTicks: finiteNonNegative(input.baseSearchTicks, "parking baseSearchTicks"),
  });
}
...
export class ParkingAuthoritySystem {
  ...
  upsert(facility: ParkingFacility): void {
    ...
    if (reservationCount > canonical.occupied) {
      throw new Error("parking occupancy cannot drop below active reservations");
    }
    this.facilities.set(canonical.id, canonical);
  }

  remove(facilityId: string): boolean {
    if ([...this.reservationByVehicle.values()].includes(facilityId)) return false;
    return this.facilities.delete(facilityId);
  }

  reserve(destinationId: string, vehicleId: string): string | null {
    if (vehicleId.length === 0) throw new Error("parking vehicleId must be non-empty");
    if (this.reservationByVehicle.has(vehicleId)) return this.reservationByVehicle.get(vehicleId) ?? null;
    ...
    this.facilities.set(selected.id, Object.freeze({ ...selected, occupied: selected.occupied + 1 }));
  ...
  release(vehicleId: string): boolean {
    ...
    this.facilities.set(facility.id, Object.freeze({ ...facility, occupied: facility.occupied - 1 }));
  ...
  destinationState(destinationId: string): ParkingDestinationState {
    ...
    const capacity = matching.reduce((total, facility) => total + facility.capacity, 0);
    const occupied = matching.reduce((total, facility) => total + facility.occupied, 0);
    const legalCapacity = legal.reduce((total, facility) => total + facility.capacity, 0);
    const legalOccupied = legal.reduce((total, facility) => total + facility.occupied, 0);
    const available = Math.max(0, legalCapacity - legalOccupied);
    const pricePerTrip = legal.length > 0 ? Math.min(...legal.map((facility) => facility.pricePerTrip)) : 0;
    const baseSearchTicks =
      legalCapacity > 0
        ? legal.reduce((total, facility) => total + facility.baseSearchTicks * facility.capacity, 0) /
          legalCapacity
        : 0;
    const occupancyRatio = legalCapacity > 0 ? legalOccupied / legalCapacity : 1;
    const searchPenaltyTicks = baseSearchTicks * (1 + occupancyRatio * 2);
    const curbRegulations = Object.freeze([...new Set(legal.map((facility) => facility.curbRegulation))].sort());
  ...
  snapshot(): ParkingAuthoritySnapshot {
    return Object.freeze({
      facilities: Object.freeze([...this.facilities.values()].sort((a, b) => a.id.localeCompare(b.id))),
      reservations: Object.freeze(
        [...this.reservationByVehicle.entries()]
          .map(([vehicleId, facilityId]) => Object.freeze({ vehicleId, facilityId }))
  ...
  restore(snapshot: ParkingAuthoritySnapshot): void {
    ...
    for (const facility of snapshot.facilities) {
      const canonical = canonicalFacility(facility);
      if (facilities.has(canonical.id)) throw new Error(`duplicate parking facility id: ${canonical.id}`);
      facilities.set(canonical.id, canonical);
    }
    const reservations = new Map<string, string>();
    const reservationCounts = new Map<string, number>();
    for (const reservation of snapshot.reservations) {
      if (reservation.vehicleId.length === 0) throw new Error("parking reservation vehicleId must be non-empty");
      if (reservations.has(reservation.vehicleId)) {
        throw new Error(`duplicate parking reservation vehicle: ${reservation.vehicleId}`);
      }
      if (!facilities.has(reservation.facilityId)) {
        throw new Error(`parking reservation references unknown facility: ${reservation.facilityId}`);
      }

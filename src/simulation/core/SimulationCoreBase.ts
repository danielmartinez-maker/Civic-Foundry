...
import {
  ...
  type BuildingV2,
} from "../buildings/BuildingTypes.ts";
import {
  BUILDING_VARIANTS,
...
import type {
  FloodEventResolvedPayload,
  FloodEventStartedPayload,
  WorldGeneratedPayload,
  WorldMigratedTo1RPayload,
...
export function withSimulationCoreHydrationOverride<T>(
...
function clampConstructionCostIndex(value: number): number {
  return Math.max(0.85, Math.min(1.5, value));
}
...
function installTerrainDevelopmentCosts(
  ...
  system.evaluateLot = (lot, definitions, context) => {
    ...
    if (!Number.isFinite(multiplier) || multiplier <= 0)
      throw new Error(
        `invalid development terrain cost multiplier at ${lot.x},${lot.y}`,
      );
    return evaluateLot(lot, definitions, {
      ...context,
      constructionCostIndex: clampConstructionCostIndex(
        context.constructionCostIndex * multiplier,
      ),
    });
  };
}
...
function footprintsMatchWithinTolerance(
  ...
  const overlapArea = polygonIntersection(left, right).reduce(
    (sum, ring) => sum + polygonArea(ring),
    0,
  );
  const symmetricDifferenceArea = leftArea + rightArea - 2 * overlapArea;
  return (
    Math.abs(symmetricDifferenceArea) <=
    CANONICAL_BUILDING_FOOTPRINT_TOLERANCE_M2
  );
}
...
export class SimulationCore extends LegacySimulationCore {
  ...
  constructor(options: SimulationCoreOptions = {}) {
    ...
    if (options.world && hydration?.world && options.world !== hydration.world)
      throw new Error("conflicting world hydration override");
    const injectedWorld = options.world ?? hydration?.world;
    const seed = options.seed ?? injectedWorld?.seed ?? 1;
    if (injectedWorld && injectedWorld.seed !== seed)
      throw new Error("world seed does not match simulation seed");

    let world: WorldFoundation;
    let generationRegistry: RandomStreamRegistry | null = null;
    const generatedHere =
      injectedWorld === undefined && options.terrain === undefined;
    if (injectedWorld) {
      if (options.terrain) {
        const compatibility = injectedWorld.legacyTerrain();
        if (
          compatibility.width !== options.terrain.width ||
          compatibility.height !== options.terrain.height
        ) {
          throw new Error(
            "hydrated world dimensions do not match compatibility terrain",
          );
        }
      }
      world = injectedWorld;
    } else if (options.terrain) {
      world = WorldFoundation.fromLegacyTerrain(
        options.terrain,
        seed,
        options.terrainMode ?? hydration?.terrainMode ?? "legacy-explicit",
      );
    } else {
    ...
    const movementIntersections = new MovementAwareIntersectionAdapter(
      transportation3R,
    );
    super({
      seed,
      terrain: world.legacyTerrain(),
      ...(options.startingFunds !== undefined
    ...
    movementIntersections.setNetworkRefresher(() => {
      this.transportation3R.refreshNetwork(
        this.roads,
        this.transportationGraph,
      );
    });
    movementIntersections.setQueuedOutgoingEdgeResolver((vehicleId) =>
      this.queuedOutgoingEdge(vehicleId),
    );
    ...
    const preparationMultiplierAt = (x: number, y: number): number =>
      this.world.preparationMultiplierAt(x, y);
    this.roads.setCostMultiplierProvider(preparationMultiplierAt);
    installTerrainDevelopmentCosts(
      this.developmentFeasibility,
      preparationMultiplierAt,
    );
    const redevelopmentFeasibility = (
      this as unknown as {
        redevelopmentFeasibility: DevelopmentFeasibilitySystem;
      }
    ...
    if (generationRegistry)
      this.kernel.random.restore(generationRegistry.snapshot());
    this.kernel.snapshots.register("world", () =>
      this.world.diagnosticSnapshot(),
    );
    this.kernel.invariants.register({
      ...
      check: () => {
        if (
          this.world.terrain.width !== this.terrain.width ||
          this.world.terrain.height !== this.terrain.height
        ) {
    ...
    if (generatedHere) {
      ...
      this.kernel.events.append(this.clock.tick, {
        type: "WorldGenerated",
        source: "world",
        payload,
      });
  ...
  override bulldozeAt(
    ...
    if (result.ok && (result.kind === "road" || result.kind === "zone"))
      this.rebuildCadastreFromLegacyState();
  ...
  private queuedOutgoingEdge(vehicleId: string): string | undefined {
    ...
    if (trafficVehicle)
      return trafficVehicle.edgeIds[trafficVehicle.currentEdgeIndex + 1];
    const serviceVehicle = this.serviceVehicles.getVehicle(vehicleId);
    if (!serviceVehicle) return undefined;
    const route =
      serviceVehicle.state === "returning"
        ? serviceVehicle.returnEdgeIds
        : serviceVehicle.edgeIds;
  ...
  protected override collectVacantDevelopmentOpportunities(
    ...
    const parcels = [...this.cadastre.listParcels()].sort((left, right) =>
    ...
    for (const parcel of parcels) {
      ...
      if (
        parcelLots.length === 0 ||
        parcelLots.some((lot) => occupiedLots.has(lot.id))
      )
        continue;
      const compatibilityLot = parcelLots[0]!;
      const legacyEvaluations = super.collectVacantDevelopmentOpportunities(
        [compatibilityLot],
        occupiedLots,
      );
      ...
      const legalDefinitionIds = new Set(
        legacyEvaluations
          ...
          .map((evaluation) => evaluation.definitionId),
      );
      ...
      const envelope = this.buildableEnvelopes.evaluate(
        parcel.id,
        this.cadastre,
        district,
      );
      const candidates = this.buildingMassing.generate(
        parcel,
        envelope,
        typologies,
      );
      const legacyByDefinitionId = new Map(
        legacyEvaluations.map(
          (evaluation) => [evaluation.definitionId, evaluation] as const,
        ),
      );
      ...
      for (const candidate of candidates) {
        const typology = typologies.find(
          (item) => item.id === candidate.typologyId,
        );
        ...
        const rentableCapacity = Math.max(
          1,
          definition.residentCapacity + definition.jobCapacity,
        );
        const prePolicyGrossPotentialRent =
          definition.baseRent *
          marketContext.marketRentMultiplier *
          rentableCapacity *
          siteScale;
        const averageMarketRentPerM2 =
          prePolicyGrossPotentialRent / candidate.usableFloorAreaM2;
        const marketRentPerM2ByUse = Object.freeze(
          Object.fromEntries(
        ...
        const constructionCostIndex =
          legacyEvaluation.hardConstructionCost /
          Math.max(
            1,
            definition.baseConstructionCost * definition.complexityFactor,
          );
        const physicalContext: PhysicalDevelopmentContext = {
          ...
          policyAffordableHousingShare:
            marketContext.policyAffordableHousingShare,
          policyDevelopmentFeeRate: marketContext.policyDevelopmentFeeRate,
          policyPermittingCostReduction:
            marketContext.policyPermittingCostReduction,
        };
        opportunities.push(
          this.developmentFeasibility.evaluateCandidate(
            candidate,
            parcel,
            physicalContext,
          ),
  ...
  protected override compatibilityAwardForLot(
    award: DevelopmentAward,
    lot: Lot,
  ): DevelopmentAward {
    if (award.lotId === lot.id) return award;
    if (!award.physicalCandidateId || !this.cadastre.getParcel(award.lotId))
      return award;
    const definition = getBuildingDefinition(award.definitionId);
    if (definition.zone !== lot.zone)
      throw new Error(
        `canonical award zone does not match compatibility lot: ${award.lotId}`,
      );
  ...
  runDesignStorm(event: DesignStormEvent): FloodResult {
    const started: FloodEventStartedPayload = {
      eventId: event.id,
      rainfallMm: event.rainfallMm,
      durationHours: event.durationHours,
    };
    this.kernel.events.append(this.clock.tick, {
      type: "FloodEventStarted",
      source: "world",
      payload: started,
    });
  ...
  recordWorldMigrationDiagnostic(fromSaveVersion: number): void {
    if (!Number.isInteger(fromSaveVersion) || fromSaveVersion < 0)
      throw new Error(
        "migration source save version must be a non-negative integer",
      );
    const payload: WorldMigratedTo1RPayload = {
      fromSaveVersion,
      mode: "legacy-flat",
    };
    this.kernel.events.append(this.clock.tick, {
      type: "WorldMigratedTo1R",
      source: "world",
      payload,
    });
  }
  ...
  private compatibilityLotsForParcel(
    parcel: Parcel,
    lots: readonly Lot[],
  ): Lot[] {
  ...
  private reconcileCanonicalBuildingProjection(): void {
    ...
    const existingById = new Map(
      existingBuildings.map((building) => [building.id, building] as const),
    );
    const consumedExistingIds = new Set<string>();
    const parcels = [...this.cadastre.listParcels()].sort((left, right) =>
      left.id.localeCompare(right.id),
    );
    const legacyBuildings = this.buildings
      .list()
      .sort((left, right) => left.id.localeCompare(right.id));
    const compatibilityLots = this.lots.list();
    const primaryLotIdByParcel = new Map<string, string>();
    for (const parcel of parcels) {
      const primaryLot = this.compatibilityLotsForParcel(
        parcel,
        compatibilityLots,
      )[0];
    ...
    for (const building of legacyBuildings) {
      ...
      const parcel = parcels.find((candidate) =>
        pointInPolygon(center, this.cadastre.parcelPolygon(candidate.id)),
      );
      if (!parcel) continue;
      const zone = legacyZoneForParcel(parcel);
      if (!zone) continue;
      ...
      const projection = projectLegacyBuildingCandidate(
        building,
        parcel,
        typology,
      );
      ...
      const isPrimary =
        !primaryAssignedParcels.has(parcel.id) &&
      ...
      if (existing) {
        ...
        const legacyStatus =
          building.status === "occupied" ? "occupied" : "construction";
        const status =
          existing.status === "construction" && legacyStatus === "occupied"
            ? "occupied"
            : existing.status;
      ...
      canonical.push(
        Object.freeze({
          ...
          entitlement: Object.freeze({
            ...
            approvedFAR: compliance.legal
              ? envelope.effectiveFAR
              : projection.realizedFAR,
            approvedHeightMeters: compliance.legal
              ? envelope.maxHeightMeters
              : projection.heightMeters,
            approvedUses: Object.freeze([
              ...(compliance.legal ? envelope.permittedUses : projection.uses),
            ]),

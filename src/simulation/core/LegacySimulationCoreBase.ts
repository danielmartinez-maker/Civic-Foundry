...
} from "../../world/roads/RoadSystem.ts";
...
} from "../utilities/UtilitySystem.ts";
import {
  GarbageSystem,
  type GarbageSnapshot,
} from "../garbage/GarbageSystem.ts";
import {
  EconomySystem,
  type EconomySnapshot,
} from "../economy/EconomySystem.ts";
...
import {
  TrafficAnalytics,
  type TrafficAnalyticsSnapshot,
} from "../traffic/TrafficAnalytics.ts";
import { ServiceFacilitySystem } from "../services/ServiceFacilitySystem.ts";
import {
  ServiceDemandSystem,
  type ServiceDemandSnapshot,
  type UnresolvedServiceInputs,
} from "../services/ServiceDemandSystem.ts";
...
} from "../services/EducationSystem.ts";
import {
  NeighborhoodQualitySystem,
  type NeighborhoodQualitySnapshot,
  type BuildingServiceAccess,
} from "../services/NeighborhoodQualitySystem.ts";
import type {
  ServiceDepartment,
  ServiceFacilityType,
...
import {
  ...
  type LandHousingMarketSnapshot,
} from "../development/LandHousingMarketSystem.ts";
import {
  RedevelopmentPressureSystem,
  type RedevelopmentPressureSnapshot,
  type ResidentialRedevelopmentInput,
} from "../development/RedevelopmentPressureSystem.ts";
import {
  RedevelopmentExecutionSystem,
  type RedevelopmentExecutionInput,
  type RedevelopmentExecutionSnapshot,
} from "../development/RedevelopmentExecutionSystem.ts";
import {
  DevelopmentPolicySystem,
  type DevelopmentPolicyPatch,
  type DevelopmentPolicyState,
} from "../development/DevelopmentPolicySystem.ts";
import type {
  DevelopmentAward,
...
import {
  HousingChoiceSystem,
  type HousingChoiceSnapshot,
} from "../housing/HousingChoiceSystem.ts";
import {
  HousingTenureSystem,
  type HousingTenureSnapshot,
} from "../housing/HousingTenureSystem.ts";
import {
  HousingRelocationSystem,
  type HousingRelocationSnapshot,
  type HousingRelocationState,
} from "../housing/HousingRelocationSystem.ts";
...
const DEPARTMENTS: readonly ServiceDepartment[] = [
  "fire",
  "police",
  "healthcare",
  "education",
...
const INTENSITY_RANK: Readonly<Record<BuildingIntensity, number>> =
...
export class LegacySimulationCore {
  ...
  serviceAccessByBuilding: Readonly<Record<string, BuildingServiceAccess>> =
    Object.freeze({});
  ...
  constructor(options: LegacySimulationCoreOptions = {}) {
    ...
    this.services = new ServiceFacilitySystem(
      this.terrain,
      this.roads,
      (x, y) =>
        ...
        this.utilities
          .listFacilities()
          .some((facility) => facility.x === x && facility.y === y),
    );
    ...
    this.transit = new TransitNetworkSystem(
      this.terrain,
      this.roads,
      (x, y) =>
        this.buildings.getAt(x, y) !== undefined ||
    ...
    this.serviceDemandSnapshot = {
      eligibleStudents: 0,
      perBuilding: Object.freeze({}),
    };
    this.educationSnapshot = {
      eligibleStudents: 0,
      reachableStudents: 0,
      enrolledStudents: 0,
      effectiveSeats: 0,
      overcrowdedStudents: 0,
      averageSchoolAccessTicks: 0,
      educationServiceRatio: 1,
    };
    this.neighborhoodSnapshot = {
      perBuilding: Object.freeze({}),
      citywideServiceQuality: 1,
      commercialServiceQuality: 1,
    };
    this.mobilitySnapshot = this.mobility.snapshot();
    this.refreshLandHousingMarket();
    this.refreshHousingTenure();
    this.housingRelocation.initialize(
  ...
  buildRoad(cells: readonly CellCoord[], type: RoadType): RoadPlacementResult {
    const occupied = cells.find((cell) =>
      this.isNonRoadOccupied(cell.x, cell.y),
    );
  ...
  paintZone(cells: readonly CellCoord[], zone: ZoneType): { painted: number } {
    const available = cells.filter(
      (cell) => !this.isNonRoadOccupied(cell.x, cell.y),
    );
  ...
  placeUtility(
    ...
    if (
      this.buildings.getAt(x, y) ||
      this.services.getAt(x, y) ||
      this.transit.getStopAt(x, y)
    ) {
  ...
  restoreHousingState(state?: HousingRelocationState): HousingRelocationState {
    ...
    if (state === undefined) {
      ...
      const buildingZones = new Map(
        this.buildings
          .occupied()
          .map((building) => [building.id, building.zone] as const),
      );
      const optionCapacities = new Map<string, number>(
        this.housingTenureSnapshot.options.map(
          (option) =>
            [`${option.buildingId}|${option.tenure}`, option.capacity] as const,
        ),
      );
      const assigned = new Map<string, number>();
      for (const allocation of state.allocations) {
        if (buildingZones.get(allocation.buildingId) !== "residential")
          throw new Error("invalid housing allocation building reference");
        const key = `${allocation.buildingId}|${allocation.tenure}`;
        if (!optionCapacities.has(key))
          throw new Error("invalid housing allocation tenure reference");
        assigned.set(key, (assigned.get(key) ?? 0) + allocation.residents);
      }
      for (const [key, residents] of assigned) {
        if (residents > (optionCapacities.get(key) ?? 0) + 1e-9)
          throw new Error("housing allocation exceeds tenure capacity");
      }
      this.housingRelocation.restoreState(state);
      const represented =
        state.allocations.reduce((sum, item) => sum + item.residents, 0) +
      ...
      if (Math.abs(represented - this.population.population) > 1e-6) {
        ...
        this.housingRelocation.refreshSnapshot(
          this.population.population,
          this.housingTenureSnapshot.options,
        );
      }
  ...
  bulldozeAt(
    ...
    const housingBeforeRemoval =
      buildingBeforeRemoval?.zone === "residential"
        ? this.housingRelocation.snapshotState()
        : undefined;
    if (buildingBeforeRemoval?.zone === "residential")
      this.housingRelocation.displaceBuilding(buildingBeforeRemoval.id);
    const building = this.buildings.removeAt(x, y);
    if (building) {
      this.economyDomain.removeBuilding(building.id, this.clock.tick);
      this.developerMarket.cancelProject(building.id, 0.5);
  ...
  private runLegacyV7Tick(): void {
    ...
    this.wasteCollection.applyJobs(
      this.serviceDispatch.listJobs(),
      this.services,
      this.clock.tick,
    );
    this.incidents.advance(
      this.clock.tick,
      this.serviceDispatch.listJobs(),
    ...
    const economyDomainSnapshot = this.economyDomain.tick({
      ...
      ...(this.clock.tick % 250 === 0
        ? { buildings: this.buildings.occupied() }
        : {}),
      population: this.population.population,
      ...
      serviceRatio:
        this.services.listFacilities().length > 0
      ...
      localDemand: Math.max(
        0.25,
        Math.min(2, this.population.population / 100),
      ),
      width: this.terrain.width,
      height: this.terrain.height,
      taxRate:
        (this.taxes.getRate("commercial") + this.taxes.getRate("industrial")) /
        2,
    });
    ...
    this.mobilitySnapshot = this.mobility.tick({
      ...
      submitCarTrip: (trip, travelerWeight, route) => {
        const freeFlowTicks = route.edgeIds.reduce(
          (sum, edgeId) =>
            sum +
            (this.transportationGraph.getEdge(edgeId)?.freeFlowTicks ?? 0),
          0,
        );
    ...
    const edgeLoads = this.mergeEdgeLoads(
  ...
  private evaluateServiceLoop(): void {
    ...
    this.lastServiceGeneratedWaste =
      this.clock.tick % 50 === 0
        ? this.wasteCollection.generate(occupied, this.clock.tick)
        : 0;
    if (this.clock.tick % 50 !== 0)
      this.wasteCollection.syncBuildings(occupied, this.clock.tick);

    const wasteByBuilding: Record<string, number> = {};
    for (const building of occupied)
      wasteByBuilding[building.id] =
        this.wasteCollection.getBuildingWaste(building.id)
          ?.currentCollectibleWaste ?? 0;
    ...
    for (const incident of this.incidents
      ...
      unresolvedByBuilding[incident.targetBuildingId] = {
        ...target,
        [key]: (target[key] ?? 0) + incident.severity,
      };
    }
    ...
    this.serviceDemandSnapshot = this.serviceDemand.evaluate(occupied, {
      ...
      unemployed: Math.max(
        0,
        this.employmentSnapshot.workforce - this.employmentSnapshot.employed,
      ),
    ...
    if (this.clock.tick % 100 === 0)
      this.incidents.generateFromDemand(
        this.clock.tick,
        occupied,
        this.serviceDemandSnapshot,
        this.serviceDispatch,
      );
    this.wasteCollection.createCollectionJobs(
      this.clock.tick,
      this.serviceDispatch,
    );
    ...
    for (const job of this.serviceDispatch
      .listJobs()
      .filter((item) => !["completed", "failed"].includes(item.status))) {
      if (job.assignedFacilityId)
        utilizationByFacility[job.assignedFacilityId] =
          (utilizationByFacility[job.assignedFacilityId] ?? 0) + 1;
    }
    for (const building of occupied) {
      ...
      for (const department of DEPARTMENTS) {
        const localDemand =
          department === "education"
            ? (demand?.educationStudents ?? 0)
            : department === "garbage"
              ? Math.max(1, wasteByBuilding[building.id] ?? 0)
              : (demand?.[department] ?? 0);
    ...
    this.educationSnapshot = this.education.evaluate(
      occupied,
      this.serviceDemandSnapshot.eligibleStudents,
      this.services,
      this.transportationGraph,
      this.pathfinding,
      (edge) => this.traffic.getEdgeTravelTime(edge),
    ...
    if (
      this.services
        .listFacilities()
        .some((facility) => facility.department === "garbage")
    ) {
      this.garbageSnapshot = this.garbage.snapshotDetailed(
        this.lastServiceGeneratedWaste,
        this.wasteCollection.processedTotal,
        this.wasteCollection.totalBacklog(),
      );
    }
  }
  ...
  private evaluateCoreCityLoop(): void {
    ...
    if (
      ...
      this.garbageSnapshot = this.garbage.evaluate(
        occupied,
        this.roads,
        this.utilities.listFacilities(),
      );
    }
    this.employmentSnapshot = this.economyDomain.snapshot(
      this.clock.tick,
    ).employment;
    ...
    const serviceQuality = hasServices
      ? this.neighborhoodSnapshot.citywideServiceQuality
      : 0.7;
    const commercialServiceQuality = hasServices
    ...
    this.demandSnapshot = this.demand.evaluate({
      ...
      trafficCommercialAccessibility:
        this.trafficSnapshot.commercialAccessibility,
      personAccessibility: this.mobilitySnapshot.personAccessibility,
      serviceQuality,
    ...
    const paymentRatio =
      this.economySnapshot.facilityOperatingCost <= 0
        ? 1
        : this.economySnapshot.paidOperatingCost /
          this.economySnapshot.facilityOperatingCost;
    ...
    const employmentQuality =
      this.employmentSnapshot.workforce === 0
        ? 1
        : this.employmentSnapshot.employed /
          Math.max(1, this.employmentSnapshot.workforce);
    const fiscalQuality =
      this.economySnapshot.unpaidOperatingCost > 0 ? paymentRatio : 1;
    const serviceFactor = hasServices
      ? this.neighborhoodSnapshot.citywideServiceQuality
      : 0.7;
    let attractiveness = clamp01(
      0.45 * essential +
        0.18 * employmentQuality +
        0.12 * fiscalQuality +
        0.25 * serviceFactor,
    );
    if (
      this.utilitySnapshot.power.serviceRatio < 0.5 ||
      this.utilitySnapshot.water.serviceRatio < 0.5
    )
      attractiveness = Math.min(attractiveness, 0.2);
    const affordabilityFactor =
      0.85 + 0.15 * this.housingChoiceSnapshot.affordabilityIndex;
    attractiveness = clamp01(attractiveness * affordabilityFactor);
    this.population.update(
      this.buildings.residentialCapacity(),
      attractiveness,
    );
    this.refreshLandHousingMarket();
    this.refreshHousingTenure();
  ...
  private evaluateDevelopmentMarket(): void {
    ...
    const occupiedLots = new Set(
      this.buildings.list().map((building) => building.lotId),
    );
    const opportunities = this.collectVacantDevelopmentOpportunities(
      lots,
      occupiedLots,
    );
    ...
    const awards = this.developerMarket.allocate(opportunities, {
      tick: this.clock.tick,
      marketInterestRate,
    });
    const lotsById = new Map(lots.map((lot) => [lot.id, lot] as const));
    const redevelopmentLotIds = new Set(
      redevelopment.opportunities.map((item) => item.lotId),
    );
    for (const award of awards) {
      ...
      try {
        if (redevelopmentLotIds.has(award.lotId)) {
          const existing = this.buildings
            .occupied()
            .find((building) => building.lotId === award.lotId);
          if (existing?.zone === "residential")
            this.housingRelocation.displaceBuilding(existing.id);
          const { removed } = this.buildings.replaceDevelopment(
            this.clock.tick,
            lot,
            compatibilityAward,
          );
          ...
          this.buildings.startDevelopment(
            this.clock.tick,
            lot,
            compatibilityAward,
          );
  ...
  protected collectVacantDevelopmentOpportunities(
    ...
    for (const lot of lots) {
      if (occupiedLots.has(lot.id) || this.demandSnapshot[lot.zone] <= 0.05)
        continue;
      opportunities.push(
        ...this.developmentFeasibility.evaluateLot(
          lot,
  ...
  protected compatibilityAwardForLot(
    award: DevelopmentAward,
    _lot: Lot,
  ): DevelopmentAward {
    return award;
  ...
  private localParcelContextForLot(lot: Lot): LocalParcelContext {
    ...
    const roadAccessBonus =
      frontage?.type === "arterial"
        ? 0.12
        : frontage?.type === "collector"
          ? 0.07
          : 0.02;
    const personAccessibility = clamp01(
      this.mobilitySnapshot.personAccessibility + roadAccessBonus,
    );
    const freightAccessibility = clamp01(
      this.trafficSnapshot.jobAccessibility + roadAccessBonus,
    );
    ...
    const serviceQuality = hasServices
      ? lot.zone === "commercial"
        ...
        : this.neighborhoodSnapshot.citywideServiceQuality
      : 0.7;
    const neighborhoodQuality = clamp01(
      serviceQuality * 0.7 + personAccessibility * 0.3,
    );
    const constructionCostIndex = clamp(
      1 + (1 - utilityRatio) * 0.2 + (1 - serviceQuality) * 0.1,
      0.85,
      1.5,
    );
    const baseZoningMaxIntensity: BuildingIntensity =
      personAccessibility >= 0.78 && utilityRatio >= 0.85
        ? "high"
        : personAccessibility >= 0.55
          ? "medium"
          : "low";
    const zoningMaxIntensity = this.developmentPolicy.adjustMaxIntensity(
      lot.zone,
      baseZoningMaxIntensity,
    );
    return {
      roadAccessBonus,
      personAccessibility,
  ...
  protected developmentContextForLot(lot: Lot): DevelopmentParcelContext {
    ...
    return {
      ...
      policyAffordableHousingShare:
        lot.zone === "residential" ? policy.affordableHousingShare : 0,
  ...
  private refreshHousingTenure(): HousingTenureSnapshot {
    const lotsById = new Map(
      this.lots.list().map((lot) => [lot.id, lot] as const),
    );
    const inputs = [];
    ...
    for (const building of this.buildings.occupied()) {
      ...
      inputs.push({
        ...
        askingRent:
          definition.baseRent * market.marketRentMultiplier * rentFactor,
        personAccessibility: local.personAccessibility,
        serviceQuality: local.serviceQuality,
        neighborhoodQuality: local.neighborhoodQuality,
        utilityRatio: local.utilityRatio,
    ...
    return this.housingTenure.evaluate(
      this.currentDevelopmentInterestRate(),
      inputs,
    );
  }
  ...
  private syncHousingPopulationWithoutVoluntaryMoves(): HousingRelocationSnapshot {
    ...
    return this.housingRelocation.refreshSnapshot(
      this.population.population,
      this.housingTenureSnapshot.options,
    );
  }
  ...
  private lowerIncomeAffordableSlackExcluding(buildingId: string): number {
    ...
    for (const option of this.housingTenureSnapshot.options) {
      if (
        ...
        housingAffordabilityScore(option.monthlyCost, "lower") <= 0
      )
        continue;
      const assigned = state.allocations
        .filter(
          (allocation) =>
            allocation.buildingId === option.buildingId &&
            allocation.tenure === option.tenure,
        )
  ...
  private refreshRedevelopmentPressure(): RedevelopmentPressureSnapshot {
    ...
    const occupiedResidential = this.buildings
      .occupied()
    ...
    for (const building of occupiedResidential) {
      ...
      const existingEvaluation = this.redevelopmentFeasibility.evaluateLot(
        lot,
        [existingDefinition],
        context,
      )[0];
      if (!existingEvaluation) continue;
      const replacements = BUILDING_VARIANTS.residential.filter(
        (candidate) =>
          INTENSITY_RANK[candidate.intensity] >
          INTENSITY_RANK[existingDefinition.intensity],
      );
      const replacementEvaluations =
        replacements.length > 0
          ? this.redevelopmentFeasibility.evaluateLot(
              lot,
              replacements,
              context,
            )
      ...
      inputs.push({
        ...
        assignedResidents:
          this.housingChoiceSnapshot.byBuilding[building.id]
            ?.assignedResidents ?? 0,
        existingEvaluation,
  ...
  private refreshRedevelopmentExecution(): RedevelopmentExecutionSnapshot {
    ...
    const committedBuildingIds = new Set(
      this.developerMarket
        .listCommitments()
        .map((commitment) => commitment.buildingId),
    );
    ...
    for (const pressure of this.redevelopmentPressure.snapshot().parcels) {
      ...
      const replacement = BUILDING_VARIANTS.residential.find(
        (candidate) => candidate.id === pressure.bestReplacementDefinitionId,
      );
      ...
      const existingDefinition = definitionForBuilding(building);
      const displacedLowerIncomeResidents = this.housingRelocation
        .snapshotState()
        .allocations.filter(
          (allocation) =>
            allocation.buildingId === building.id &&
            allocation.band === "lower",
        )
        .reduce((sum, allocation) => sum + allocation.residents, 0);
      inputs.push({
        pressure,
        ...
        lowerIncomeAffordableSlack: this.lowerIncomeAffordableSlackExcluding(
          building.id,
        ),
    ...
    return this.redevelopmentExecution.evaluate(
      {
        ...
        effectiveAffordableCapacity:
          this.housingChoiceSnapshot.effectiveAffordableCapacity,
        unplacedResidents: this.housingChoiceSnapshot.unplacedResidents,
        minimumAffordableShare:
          this.developmentPolicy.snapshot().redevelopmentAffordableFloor,
        lowerIncomeRelocationProtection:
          this.developmentPolicy.snapshot().lowerIncomeRelocationProtection,
      },
      inputs,
    );
  }
  ...
  private refreshLandHousingMarket(): LandHousingMarketSnapshot {
    ...
    return this.landHousingMarket.evaluate({
      ...
      employmentUtilization:
        this.employmentSnapshot.totalJobs === 0
          ? 0
          : this.employmentSnapshot.employed /
            this.employmentSnapshot.totalJobs,
      personAccessibility: this.mobilitySnapshot.personAccessibility,
  ...
  private currentDevelopmentInterestRate(): number {
    return clamp(
      0.045 + Math.max(0, this.economySnapshot.unpaidOperatingCost) / 1_000_000,
      0.03,
      0.12,
    );
  }
  ...
  private roadRemovalWouldStrandOccupant(x: number, y: number): boolean {
    ...
    for (const occupant of occupants) {
      const adjacent = CARDINAL.map(([dx, dy]) => ({
        x: occupant.x + dx,
        y: occupant.y + dy,
      }));
      if (!adjacent.some((cell) => cell.x === x && cell.y === y)) continue;
      const hasAlternative = adjacent.some(
        (cell) =>
          (cell.x !== x || cell.y !== y) && this.roads.has(cell.x, cell.y),
      );
  ...
  private mergeEdgeLoads(
    ...sources: Readonly<Record<string, number>>[]
  ): Record<string, number> {
    const result: Record<string, number> = {};
    for (const source of sources) {
      for (const [edgeId, load] of Object.entries(source))
        result[edgeId] = (result[edgeId] ?? 0) + Math.max(0, load);
    }
    return result;
  }
}

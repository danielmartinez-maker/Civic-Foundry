...
type FirmAccess=Readonly<{firm:Firm;building:Building;nodeId:string}>;
...
export class EconomyScheduler{
  ...
  tick(input:EconomyTickInputs):EconomyDomainSnapshot{
    ...
    if(input.tick%ECONOMY_CADENCE.replenishment===0){this.freightDemand.createReplenishmentOrders(this.firms.list(),this.inventories,input.tick);this.createExportOrders(input.tick);this.dispatchWaitingOrders(input);}
  ...
  removeBuilding(buildingId:string,tick:number):void{const firm=this.firms.getByBuildingId(buildingId);if(!firm)return;this.firms.update(firm.id,{status:'closed',closureTick:tick,filledJobs:0,vacancies:0,distressReason:'building removed'});this.freightDemand.cancelForFirm(firm.id);this.inventories.removeFirm(firm.id);this.businessClosures++;}
  ...
  private runLifecycle(input:EconomyTickInputs):void{
    ...
    for(const firm of this.firms.list().filter(f=>f.status==='forming')){
      const access=this.accessForFirm(firm,input.graph);
      const reachable=!!access&&gateways.some(g=>input.pathfinding.findRoute(input.graph,access.nodeId,g.nodeId,{costKey:'freight-free-flow'})!==null);
  ...
  private createExportOrders(tick:number):void{const gateways=this.trade.listGateways();if(gateways.length===0)return;for(const firm of this.firms.list().filter(f=>f.status==='operating'&&(f.archetype==='light_manufacturing'||f.archetype==='assembly_manufacturing'))){const inv=this.inventories.get(firm.id,'manufactured_goods');const existing=this.freightDemand.listOrders().some(o=>o.originFirmId===firm.id&&o.destinationKind==='gateway'&&(o.status==='waiting'||o.status==='dispatched'));const surplus=Math.max(0,inv.onHand-Math.max(5,inv.targetStock));if(!existing&&surplus>=5)this.freightDemand.createExportOrder(firm.id,'manufactured_goods',Math.min(10,surplus),gateways[0]!.id,tick);}}
  private dispatchWaitingOrders(input:EconomyTickInputs):void{for(const order of this.freightDemand.listOrders().filter(o=>o.status==='waiting').sort((a,b)=>b.priority-a.priority||a.id.localeCompare(b.id))){const destNode=this.destinationNode(order,input.graph);if(!destNode)continue;const candidates=this.candidates(order);const match=this.freightDemand.matchOrder(order,candidates,c=>{const originNode=this.candidateNode(c.id,c.kind,input.graph);if(!originNode)return Infinity;const route=input.pathfinding.findRoute(input.graph,originNode,destNode,{costKey:'freight-free-flow'});return route?this.routeCost(route,input):Infinity;});if(!match)continue;this.dispatchMatch(order,match,destNode,input);}}
  private dispatchMatch(order:FreightOrder,match:FreightMatch,destNode:string,input:EconomyTickInputs):void{const originNode=this.candidateNode(match.originId,match.originKind,input.graph);if(!originNode)return;const route=input.pathfinding.findRoute(input.graph,originNode,destNode,{costKey:'freight-free-flow'});if(!route||route.edgeIds.length===0)return;const shipmentId=`shipment:${this.nextShipmentId++}`;try{if(match.originKind==='firm')this.inventories.dispatchCargo(match.originId,match.commodity,match.quantity,shipmentId);else this.inventories.createExternalCargo(match.commodity,match.quantity,shipmentId);}catch{return;}const shipment:FreightShipment={id:shipmentId,orderId:order.id,commodity:match.commodity,quantity:match.quantity,vehicleWeight:Math.max(1,match.quantity/10),originKind:match.originKind,originId:match.originId,destinationKind:match.destinationKind,destinationId:match.destinationId,originNodeId:originNode,destinationNodeId:destNode,createdTick:input.tick,generalizedCost:match.generalizedCost};this.freightVehicles.dispatch(shipment,route,input.tick);this.freightDemand.markDispatched(order.id,shipmentId);this.logisticsCostTotal+=match.generalizedCost;this.logisticsShipments++;const costTarget=match.destinationKind==='firm'?match.destinationId:(order.originFirmId??'');if(costTarget){const acc=this.accrual(costTarget);acc.logisticsCost+=match.generalizedCost*0.1;this.financials.set(costTarget,acc);}}
  ...
  private destinationNode(order:FreightOrder,graph:TransportationGraph):string|undefined{return order.destinationKind==='gateway'?this.trade.getGateway(order.destinationId)?.nodeId:this.accessForFirm(this.firms.get(order.destinationId),graph)?.nodeId;}
  private candidateNode(id:string,kind:'firm'|'gateway',graph:TransportationGraph):string|undefined{return kind==='gateway'?this.trade.getGateway(id)?.nodeId:this.accessForFirm(this.firms.get(id),graph)?.nodeId;}
  private accessForFirm(firm:Firm|undefined,graph:TransportationGraph):FirmAccess|undefined{if(!firm)return undefined;const building=this.cachedBuildings.get(firm.buildingId);if(!building)return undefined;const ids=[[0,-1],[1,0],[0,1],[-1,0]].map(([dx,dy])=>graph.findNodeAt(building.x+dx!,building.y+dy!)?.id).filter((id):id is string=>id!==undefined).sort();return ids[0]?{firm,building,nodeId:ids[0]}:undefined;}

...
export class DemandSystem {
  evaluate(input: DemandInputs): DemandSnapshot {
    ...
    const residential = 0.3 * housingPressure + 0.25 * employmentQuality + 0.2 * service + 0.15 * taxSignal(input.taxRates.residential) + 0.1 * (input.trafficJobAccessibility * 2 - 1);
    const commercial = 0.35 * commercialSupply + 0.2 * service + 0.15 * taxSignal(input.taxRates.commercial) + 0.15 * employmentQuality + 0.15 * (input.trafficCommercialAccessibility * 2 - 1);

...
export type ServiceVehicleState =
  "idle" | "outbound" | "servicing" | "returning" | "unavailable";
...
const isEmergency = (type: ServiceVehicleType): boolean =>
  type !== "garbage_truck";
...
export class ServiceVehicleSystem {
  ...
  syncFleet(facilities: ServiceFacilitySystem): void {
      ...
      for (let i = 1; i <= definition.baseVehicleCount; i++) {
        ...
        if (!existing) {
          this.vehicles.set(id, {
            ...
            edgeProgressTicks: 0,
            currentSpeed: 0,
            state: shouldBeAvailable ? "idle" : "unavailable",
    ...
    for (const id of [...this.vehicles.keys()])
      if (!valid.has(id)) this.vehicles.delete(id);
  }

  listVehicles(): ServiceVehicle[] {
    return [...this.vehicles.values()]
      .map((vehicle) => this.copy(vehicle))
  ...
  restore(vehicles: readonly ServiceVehicle[]): void {
    ...
    for (const vehicle of vehicles) {
      const restored: MutableVehicle = {
        ...
        returnEdgeIds: [...vehicle.returnEdgeIds],
      };
  ...
  step(
    ...
    for (const vehicle of this.vehicles.values()) {
      ...
      if (
        (vehicle.state === "outbound" || vehicle.state === "returning") &&
        !this.ensureValidRoute(vehicle, graph, pathfinding, edgeCost, tick)
      ) {
        if (vehicle.currentJobId)
          events.push({
            type: "failed",
    ...
    for (const vehicle of [...this.vehicles.values()].sort((a, b) =>
  ...
  edgeLoads(): Record<string, number> {
    ...
    for (const vehicle of this.vehicles.values()) {
      ...
      result[edgeId] =
        (result[edgeId] ?? 0) +
        (vehicle.vehicleType === "garbage_truck" ? 2 : 1);
    }
    return result;
  }

  private ensureValidRoute(
    ...
    const reroute = pathfinding.findRoute(graph, start, end, {
      ...
      costKey: `service-reroute:${vehicle.vehicleType}:${Math.floor(tick / 10)}`,
    });
  ...
  private edgeTravelTicks(
    vehicle: MutableVehicle,
    edge: TransportationEdge,
    edgeCost: (edge: TransportationEdge) => number,
  ): number {
  ...
  private copy(vehicle: MutableVehicle): ServiceVehicle {
    const base: ServiceVehicle = {
      ...
      currentJobId: vehicle.currentJobId,
      edgeIds: [...vehicle.edgeIds],
      returnEdgeIds: [...vehicle.returnEdgeIds],
      ...
      destinationNodeId: vehicle.destinationNodeId,
      homeNodeId: vehicle.homeNodeId,
      serviceRemainingTicks: vehicle.serviceRemainingTicks,

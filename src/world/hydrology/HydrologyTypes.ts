...
export type HydrologySnapshot = Readonly<{
  width: number; height: number; conditionedElevationMeters: readonly number[]; receiver: readonly (number | null)[];
  watersheds: readonly WatershedRecord[]; channels: readonly ChannelSegment[]; flowAccumulation: readonly number[];
  watershedIds: readonly WatershedId[]; floodSusceptibility: readonly number[];
}>;
...
export type FloodResult = Readonly<{
  eventId: string; depthMeters: readonly number[]; rainfallVolume: number; infiltrationVolume: number;
  retainedChannelSurfaceVolume: number; overbankFloodVolume: number; exportedVolume: number; balanceError: number;
}>;

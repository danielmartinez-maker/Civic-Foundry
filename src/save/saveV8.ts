...
export function hydrateCoreV8(input: unknown): SimulationCore {
  if (!isRecord(input) || input.saveVersion !== 8) {
    return withSimulationCoreHydrationOverride({ terrainMode: 'legacy-flat' }, () => hydrateCoreV7(input));
  }
...
function validateCompatibilityTerrain(
  saved: Readonly<{ width: number; height: number; cells: readonly TerrainCell[] }>,
  ...
  for (let index = 0; index < expectedCells.length; index++) {
    ...
    if (!a || !b || a.elevation !== b.elevation || a.water !== b.water || a.buildable !== b.buildable || a.biome !== b.biome) {
